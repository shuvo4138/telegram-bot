...
async def api_get_number_s2(range_val, app_name="FACEBOOK"):
    ...
    try:
        ...
        if login_data.get("meta", {}).get("code") != 200:
            logging.error(f"❌ X.Mint login failed: {login_data.get('meta')}")
        ...
        logging.info(f"✅ X.Mint: Got token, range: {clean_range}")
        
        # StexSMS style payload
        payload = {
            "range": clean_range,
            "is_national": False,
            "remove_plus": True,
...
async def api_get_info_s2(search="", status=""):
    """X.Mint থেকে OTP info নাও - Simple direct login"""
    try:
        # Fresh login
        async with httpx.AsyncClient(timeout=15) as client:
            login_res = await client.post(
...
async def post_init(application):
    ...
    logging.info("✅ StexSMS Pool (50/50) initialized! X.Mint uses direct login.")
