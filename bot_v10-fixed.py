...
# User Database Channel (redeploy এর পরেও user list থাকবে)
...
user_data = {}
user_locks = {}  # Per-user locks for thread-safe access
user_msg = {}    # প্রতি user এর current message_id track করো
user_range_msg = {}  # range select message track করো
user_kb_msg = {}    # reply keyboard message track করো

# User DB message_id tracker
user_db_msg_id = {}  # { user_id: message_id }
...
def _load_existing_users():
    for u in _EXISTING_USERS:
        ...
        user_data[uid].setdefault("carrier", None)
...
class SessionPool:
    ...
    async def initialize(self):
        async with self.lock:
            if self.initialized:
                return
            logging.debug("🔄 S1 Session pool initialize হচ্ছে...")
            results = []
    ...
    async def refresh_all(self):
        logging.debug("🔄 Session pool refresh হচ্ছে...")
        async with self.lock:
            self.initialized = False
...
class XMintSessionPool:
    ...
    async def initialize(self):
        async with self.lock:
            if self.initialized:
                return
            logging.debug("🔄 S2 (X.Mint) Session pool initialize হচ্ছে...")
            results = []
            for i in range(50):
                r = await self._login_once()
                results.append(r)
    ...
    async def refresh_all(self):
        logging.debug("🔄 X.Mint Session pool refresh হচ্ছে...")
        async with self.lock:
            self.initialized = False
            while not self.number_sessions.empty():
                try:
...
async def api_get_number_s2(range_val, app_name="FACEBOOK", _retry=0):
    logging.debug(f"🔵 X.Mint: Getting number for {app_name}, range: {range_val}")
    
    clean_range = ''.join(c for c in range_val.upper() if c.isdigit() or c == 'X')
    if not clean_range:
        return {"error": "Invalid range"}, None
    # XXX ensure করো — exactly 3X দিয়ে শেষ হবে
    ...
    try:
        ...
        logging.debug(f"🔵 S2 number response: HTTP {res.status_code}")

        if res.status_code == 403:
            logging.warning("⚠️ S2 number: 403 — session expired, relogin করা হচ্ছে")
            new_session = await xmint_pool._login_once()
            if new_session.get("token") and _retry < 2:
                logging.debug("🔄 S2: নতুন session দিয়ে retry করা হচ্ছে")
                await asyncio.sleep(1)
        ...
        if res.status_code == 429 or res.status_code == 503:
            ...
            logging.warning(f"⚠️ S2 number: HTTP {res.status_code} — {wait_time}s পর retry")
        ...
        if result.get("meta", {}).get("code") == 200:
            logging.debug(f"✅ S2 number success!")
        else:
            msg = str(result.get("message", "")).lower()
            if any(k in msg for k in ["block", "rate", "limit", "many", "temporary"]):
                logging.warning(f"⚠️ S2 Rate limited: {msg} — retry {_retry}")
                await xmint_pool.return_number_session(session)
                if _retry < 3:
                    wait_time = 10 * (2 ** _retry)
                    logging.debug(f"⏳ S2: {wait_time}s wait")
                    await asyncio.sleep(wait_time)
                    return await api_get_number_s2(range_val, app_name, _retry=_retry + 1)
                return result, None
...
async def send_otp_to_channel(bot, number, otp, app, country, flag, raw_sms="", panel="S1"):
    try:
        ...
        logging.debug(f"✅ Channel OTP - {app_cap} ({country})")
...
async def get_xmint_console_logs(force=False):
    ...
    try:
        ...
        if not token:
            logging.warning("X.Mint Console: token পাওয়া যায়নি")
            return _xmint_console_cache["logs"]
        ...
        if res.status_code != 200:
            logging.warning(f"X.Mint Console: HTTP {res.status_code} | body: {res.text[:200]}")
            return _xmint_console_cache["logs"]
        if not res.text.strip():
            logging.warning("X.Mint Console: empty response")
            return _xmint_console_cache["logs"]
        try:
            data = res.json()
        except Exception as je:
            logging.warning(f"X.Mint Console: Invalid JSON — {je} | raw: {res.text[:200]}")
            return _xmint_console_cache["logs"]
        if data.get("meta", {}).get("code") == 200:
            logs = data["data"].get("logs", [])
            _xmint_console_cache = {"logs": logs, "time": time.time()}
            return logs
        logging.warning(f"X.Mint Console: API error — {data.get('message', '')}")
        return _xmint_console_cache["logs"]
    except Exception as e:
        logging.error(f"X.Mint Console error: {type(e).__name__}: {e}")
        return _xmint_console_cache["logs"]
...
async def api_get_number(range_val, app_name="FACEBOOK", _retry=0):
    ...
    try:
        ...
        if res.status_code == 403:
            logging.warning("⚠️ S1 number: 403 — session expired, relogin করা হচ্ছে")
            new_session = await session_pool._login_once()
            if new_session.get("token") and _retry < 2:
                logging.debug("🔄 S1: নতুন session দিয়ে retry করা হচ্ছে")
                await asyncio.sleep(1)
        ...
        if res.status_code == 429 or res.status_code == 503:
            wait_time = 10 * (2 ** _retry)
            logging.warning(f"⚠️ S1 number: HTTP {res.status_code} — {wait_time}s পর retry")
        ...
        if any(k in msg for k in ["block", "rate", "limit", "many", "temporary"]):
            logging.warning(f"⚠️ S1 Rate limited: {msg} — retry {_retry}")
            await session_pool.return_number_session(session)
            if _retry < 3:
                wait_time = 10 * (2 ** _retry)
                logging.debug(f"⏳ S1: {wait_time}s wait")
                await asyncio.sleep(wait_time)
                return await api_get_number(range_val, app_name, _retry=_retry + 1)
            return data, None
        return data, session
...
async def db_load_all_users(bot):
    loaded = 0
    try:
        logging.debug("📥 DB Channel থেকে user data load হচ্ছে...")
...
def main_keyboard(user_id=None):
    buttons = [
        ...
        [KeyboardButton("⏭️ Bulk Number"), KeyboardButton("🛟 Support Admin")],
    ]
...
async def get_dynamic_apps():
    """S1 + S2 console থেকে available apps আনো, max 5টা random"""
    try:
...
async def app_select_inline_dynamic():
    """Facebook fixed (S1/S2 step আছে) + console থেকে max 5টা dynamic"""
    ...
    for c in countries:
        if isinstance(c, dict):
            ...
            buttons.append([InlineKeyboardButton(
                f"{flag} {country} [{panel}]", callback_data=f"country_{panel}_{country}"
            )])
        else:
            flag = get_flag(c)
            buttons.append([InlineKeyboardButton(
    ...
    async def _run():
        ...
        while not stop_event.is_set():
            ...
            if elapsed >= OTP_TIMEOUT:
                logging.debug(f"⏰ OTP timeout — user {user_id}")
                stop_event.set()
                break
...
async def do_get_number(message, user_id, count=1, user_name="User", bot=None):
    ...
    if count == 1:
        ...
        if data.get("meta", {}).get("code") == 200:
            ...
            logging.debug(f"✅ Number extracted: {number}")
...
async def cmd_apistatus(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    s1_init = "✅ Ready" if session_pool.initialized else "⏳ Initializing..."
    ...
    s2_init = "✅ Ready" if xmint_pool.initialized else "⏳ Initializing..."
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("select_app_"):
        ...
        if app_name == "FACEBOOK":
            ...
            if not countries:
                await safe_edit(query,
                    f"❌ {display} এর জন্য কোনো active country নেই।",
...
# Known button texts — handle_message এ handle হয়
_KNOWN_TEXTS = {
    "📞 Get Number", "📡 Custom Range", "⏭️ Bulk Number", "🛟 Support Admin",
    "📋 My Numbers", "👑 Admin Panel", "✧ Custom Range", "🎯 Custom Range",
    "🟩 Custom Range", "🟨 Cs Range", "✧ My Numbers", "🟦 Bulk Number",
    "🕹️ Bulk Service", "✧ Bulk Service", "📦 Bulk Number", "✧ Bulk Number",
    "✧ Start", "🏠 Start", "🟩 Start",
}
...
async def auto_menu_restore(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Inline menu হারিয়ে গেলে restore করে। Known button বা command হলে skip।"""
    if not update.message or not update.message.text:
        return

    text = update.message.text.strip()

    # command বা known button হলে skip — handle_message handle করবে
    if text.startswith("/") or text in _KNOWN_TEXTS:
        return
    ...
    # waiting_for state এ থাকলে skip — custom range input ইত্যাদি
    if user_data[user_id].get("waiting_for"):
        return
    ...
    if last_msg_id:
        try:
            # inline menu আছে — directly restore করো (dummy edit ছাড়া)
            ...
            return  # inline restored, handle_message unknown reply দেবে keyboard সহ
        except Exception:
            pass  # message নেই — handle_message unknown reply দেবে
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text in ("⏭️ Bulk Number", "🟦 Bulk Number", "🕹️ Bulk Service", "✧ Bulk Service", "📦 Bulk Number", "✧ Bulk Number"):
        if not has_get100_access(user_id):
            await update.message.reply_text(
    ...
    # ❓ Unknown message — ReplyKeyboard বাটন restore করো
    await update.message.reply_text(
        "🔄 Keyboard restored!",
        reply_markup=main_keyboard(user_id)
    )
...
async def post_shutdown(application):
    ...
    logging.warning("✅ All tasks cancelled cleanly.")
