    ...
    try:
        ...
        for panel_label, logs in [("S1", s1_logs), ("S2", s2_logs)]:
            for log in logs:
                ...
                # OTP extract করো raw SMS থেকে
                otp = extract_otp(raw_sms) or "******"

                # range এ XXX না থাকলে যোগ করো
...
async def db_load_all_users(bot):
    ...
    try:
        ...
        for msg_id in range(max(1, marker_id - 2000), marker_id):
            try:
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ✅ Bug Fix — emoji সহ বা ছাড়া যেকোনো button press হলে waiting_for reset করো
    text_clean = text.strip()
    # emoji ছাড়া শুধু text অংশ বের করো
    import unicodedata
    def strip_emoji(s):
        return ''.join(c for c in s if unicodedata.category(c) not in ('So', 'Sm') and ord(c) < 0x2600 or c.isalpha() or c.isspace()).strip()

    text_no_emoji = ''.join(c for c in text_clean if c.isascii() or '\u0980' <= c <= '\u09FF').strip()

    BUTTON_KEYWORDS = [
        "get number", "custom range", "bulk number",
        "live traffic", "support admin", "cs range"
    ]
    is_button = any(kw in text_no_emoji.lower() for kw in BUTTON_KEYWORDS)
    if is_button and waiting == "custom_range":
    ...
    if text in ("📞 Get Number", "Get Number", "📞Get Number")  or text_no_emoji.strip() == "Get Number":
    ...
    if text in ("📡 Custom Range", "🟨 Cs Range", "✧ Custom Range", "🎯 Custom Range", "🟩 Custom Range", "Custom Range", "Cs Range") or "custom range" in text_no_emoji.lower() or "cs range" in text_no_emoji.lower():
        panel = user_data[user_id].get("panel", "S1")
        app = user_data[user_id].get("app", "FACEBOOK")
        cancel_all_otp_tasks(user_id)
        user_data[user_id]["otp_active"] = False
    ...
    if text in ("⏭️ Bulk Number", "🟦 Bulk Number", "📦 Bulk Number", "Bulk Number") or "bulk number" in text_no_emoji.lower():
        if not has_get100_access(user_id):
    ...
    # 🚦 Live Traffic button
    if text in ("🚦 Live Traffic", "Live Traffic") or "live traffic" in text_no_emoji.lower():
    ...
    if text in ("🛟 Support Admin", "Support Admin") or "support admin" in text_no_emoji.lower():
        await update.message.reply_text(
