...
# Range Post Job - Concurrent limit
MAX_CONCURRENT_RANGE_JOBS = 3
...
COUNTRY_NAME_TO_CODE = {
    ...
    "central africa": "CF", "african republic": "CF", "repubblica centrafricana": "CF",
...
def get_flag(code):
    ...
    # Special case for "Central African"
    if "central" in name_key and "african" in name_key:
        return COUNTRY_FLAGS.get("CF", "🌍")
    ...
    for prefix in ["the ", "new ", "south ", "north ", "east ", "west "]:
        if name_key.startswith(prefix):
            clean = name_key.replace(prefix, "").strip()
            if clean in COUNTRY_NAME_TO_CODE:
                code_result = COUNTRY_NAME_TO_CODE[clean]
                return COUNTRY_FLAGS.get(code_result, "🌍")
...
async def db_load_all_users(bot):
    ...
    try:
        ...
        for msg_id in range(max(1, marker_id - 3000), marker_id):
            try:
                ...
                continue
        logging.warning(f"✅ DB Load complete — {loaded} users loaded (from 3000 messages)")
    except Exception as e:
