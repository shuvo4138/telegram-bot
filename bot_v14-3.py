...
async def do_get_number(message, user_id, count=1, user_name="User", bot=None):
    ...
    if data.get("meta", {}).get("code") == 200:
        ...
        # Auto next range — same country তে অন্য range try করো
        app = user_data[user_id].get("app", "FACEBOOK")
        ...
        if country:
            ...
            if next_range:
                ...
                await message.reply_text(
                    f"⚠️ Range `{range_val}` শেষ!\n⏳ Next range try হচ্ছে: `{next_range}`...",
                    parse_mode="Markdown"
                )
                await do_get_number(message, user_id, bot=bot)
                return
            else:
                # সব range শেষ — Change Country বাটন দেখাও
                user_data[user_id]["tried_ranges"] = set()
                await message.reply_text(
                    f"❌ *{country}* তে সব range শেষ!\n\nঅন্য country select করুন।",
                    parse_mode="Markdown",
