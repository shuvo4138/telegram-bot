...
COUNTRY_NAME_TO_CODE = {
    ...
    "guinea": "GN", "guinea republic": "GN", "equatorial guinea": "GQ",
    "guinea-bissau": "GW", "zambia": "ZM", "zimbabwe": "ZW",
    ...
    "central africa": "CF", "centrafrique": "CF",
    "gabon": "GA", "djibouti": "DJ", "eritrea": "ER", "gambia": "GM",
    "cape verde": "CV", "comoros": "KM", "seychelles": "SC", "mauritius": "MU",
    ...
    "palestine": "PS", "armenia": "AM", "azerbaijan": "AZ", "georgia": "GE",
    "kazakhstan": "KZ", "uzbekistan": "UZ", "turkmenistan": "TM",
    "kyrgyzstan": "KG", "mongolia": "MN", "russia": "RU", "ukraine": "UA",
    "belarus": "BY", "moldova": "MD", "romania": "RO", "bulgaria": "BG",
    "serbia": "RS", "croatia": "HR", "bosnia": "BA", "north macedonia": "MK",
    ...
    "norway": "NO", "finland": "FI", "iceland": "IS", "australia": "AU",
    "new zealand": "NZ", "japan": "JP", "south korea": "KR", "korea": "KR",
    "china": "CN", "taiwan": "TW", "hong kong": "HK", "macau": "MO",
    "macao": "MO", "south sudan": "SS",
    ...
    for suffix in [" republic", " kingdom", " federation", " state", " emirate", " islamic"]:
        if name_key.endswith(suffix):
            ...
            if clean in COUNTRY_NAME_TO_CODE:
                return COUNTRY_FLAGS.get(COUNTRY_NAME_TO_CODE[clean], "🌍")
...
async def job_post_live_sms(context):
    """প্রতি 60 সেকেন্ডে Console থেকে নতুন Facebook SMS channel এ post করো"""
    global _posted_sms_ids, _job_is_running
    
    # একসাথে দুটো job run হলে skip করো
    if _job_is_running:
        logging.warning("⚠️ job_post_live_sms: একটি job আগেই চলছে, এই execution skip করা হচ্ছে")
        return
    ...
    try:
        ...
        s1_logs = await asyncio.wait_for(get_console_logs(force=True), timeout=15)
        s2_logs = await asyncio.wait_for(get_xmint_console_logs(force=True), timeout=15)
        ...
        for panel_label, logs in [("S1", s1_logs), ("S2", s2_logs)]:
            for log in logs:
                ...
                import re as _re2
                star_match = _re2.search(r'\*+', raw_sms)
                otp_digits = len(star_match.group()) if star_match else 6
                otp_digits = max(4, min(6, otp_digits))
                fake_otp = ''.join([str(random.randint(0, 9)) for _ in range(otp_digits)])
        ...
        # সব message একসাথে পাঠাও (বাট timeout সহ)
        for msg, keyboard in messages_to_send:
            try:
                ...
                logging.warning(f"⚠️ Message send timeout, skipping")
            except Exception as e:
        ...
        logging.debug(f"✅ job_post_live_sms: {len(messages_to_send)} messages posted")

    except asyncio.TimeoutError:
        logging.error("⚠️ job_post_live_sms timeout — logs fetch হয়নি")
    except Exception as e:
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Unknown text — শুধু keyboard restore করো, কোনো message না
    try:
        await update.message.delete()
    except Exception:
        pass
