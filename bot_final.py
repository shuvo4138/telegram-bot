...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    elif data == "admin_s3_add":
        user_data[user_id]["waiting_for"] = "s3_number"
        await query.answer()
        await context.bot.send_message(
            chat_id=query.message.chat.id,
            text="➕ S3 Number দিন (একটি বা .txt file upload করুন):"
        )

    elif data == "admin_s3_status":
        await query.answer()
        pool_info = f"📊 S3 Pool Status:\n\n"
        pool_info += f"📦 Available: {len(hadi_numbers_pool)} numbers\n"
        pool_info += f"👥 Active sessions: {len(hadi_sessions)}\n"
        countries = {}
        for n in hadi_numbers_pool:
            c, _ = detect_country_from_number(str(n))
            countries[c] = countries.get(c, 0) + 1
        for c, cnt in sorted(countries.items(), key=lambda x: -x[1])[:10]:
            flag = get_flag(c)
            pool_info += f"  {flag} {c}: {cnt}\n"
        await context.bot.send_message(chat_id=query.message.chat.id, text=pool_info)
