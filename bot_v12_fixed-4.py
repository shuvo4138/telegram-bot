...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "select_panel_A2_fb":
        ...
        user_data[user_id]["app"]   = "FACEBOOK"
        await safe_edit(query, "⏳ লোড হচ্ছে...")
        ranges = _a2_ranges_cache["data"] or await a2_get_active_ranges(force=True)
        ...
        if not ranges:
            ...
            return
        # সরাসরি সব range দেখাও — country filter নেই, max 40
        buttons = []
        seen = set()
        for r in ranges:
            if len(buttons) >= 40:
        ...
        await safe_edit(query,
            f"📡 *Range select করুন ({len(seen)} available):*",
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup(buttons)
        )
        return
