...
async def job_a2_range_post(context):
    ...
    try:
        ...
        for r in ranges:
            ...
            header_line = f"{range_flag} {country_name} FB {display_range} [A2]"
            text = escape_mdv2(header_line)

            kb_rows = []
            kb_rows.append([InlineKeyboardButton(
                f"{otp}", copy_text=CopyTextButton(text=otp),
                api_kwargs={"style": "success"}
            )])
            bottom_row = []
...
async def send_otp_to_channel(bot, number, otp, app, country, flag, raw_sms="", panel="S1"):
    try:
        ...
        svc_emoji = "💬" if "whatsapp" in app.lower() else ("📸" if "instagram" in app.lower() else "💬")
        svc_emoji = "📘" if "facebook" in app.lower() else svc_emoji

        msg = (
            f"{flag} {escape_mdv2(country)}\n\n"
            f"📞 `{escape_mdv2(hidden_num)}`\n"
            f"🔐 `{escape_mdv2(otp)}`\n"
            f"{svc_emoji} Service: {escape_mdv2(app_cap)} \\[{escape_mdv2(panel)}\\]\n"
            f"{escape_mdv2('────────────')}\n"
            f"📩"
        )
        if raw_sms:
    ...
    try:
        ...
        for panel_label, logs in [("S1", s1_logs), ("S2", s2_logs)]:
            ...
            for log in logs:
                ...
                # ── Keyboard: Main Channel + Number Bot (no OTP box) ──
                kb_rows = []
                _kb_btm = []
