...
async def a2_get_active_ranges():
    """VOLTX active ranges fetch — /liveaccess endpoint."""
    global _a2_ranges_cache
    if (time.time() - _a2_ranges_cache["time"]) < A2_CACHE_TTL and _a2_ranges_cache["data"]:
    ...
    try:
        ...
        logger.info(f"A2 liveaccess raw: {res.text[:500]}")
        if res.status_code == 200:
            data = res.json()
            ...
            if data.get("meta", {}).get("code") == 200:
                ...
                for svc in services:
                    ...
                    if "FACEBOOK" not in sid:
                        continue
                    for rng in svc.get("ranges", []):
                        clean = rng.upper().replace("XXX", "").replace("X", "").strip()
                        ranges_out.append({
                            "range": rng,
                            "rid": clean,
                            "service": sid,
                ...
                logger.info(f"A2 liveaccess parsed ranges (FACEBOOK only): {len(ranges_out)}")
...
async def a2_get_number(rid):
    ...
    clean_rid = str(rid).upper().replace("XXX", "").replace("X", "").strip()
    try:
        async with httpx.AsyncClient(timeout=20) as client:
            res = await client.post(
...
async def a2_poll_otp(target_number):
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            if data.get("meta", {}).get("code") == 200:
                otps = data.get("data", {}).get("otps", [])
                clean_target = str(target_number).lstrip("+").strip()
                for entry in otps:
                    num = str(entry.get("number", "")).lstrip("+").strip()
                    if num == clean_target or num.endswith(clean_target[-7:]):
                        return entry
        return None
    except Exception as e:
        logger.error(f"A2 poll_otp error: {e}")
        return None
...
async def do_get_number_a2(message, user_id, bot=None):
    ...
    rid_clean = range_val.upper().replace("XXX", "").replace("X", "").strip()

    nums_got = []
    for attempt in range(2):
        result = await a2_get_number(rid_clean)
        if result:
            full_num = (result.get("no_plus_number") or result.get("national_number") or
                        result.get("full_number", "")).replace("+", "").strip()
    ...
    display_range = rid_clean + "XXX"
    app_emoji = APP_EMOJIS.get(app_name.upper(), "📱")
    card_lines = [f"✅ <b>Numbers Found!</b>  [A2]\n"]
    card_lines.append(f"{app_emoji} <b>{app_name.capitalize()}</b>  |  📡 <code>{display_range}</code>\n")
    for n in nums_got:
        card_lines.append(f"📱 <code>{n}</code>")
    ...
    user_data[user_id]["auto_otp_cancel"] = False
    task = asyncio.create_task(auto_otp_a2(user_id, nums_got, display_range, _bot, chat_id))
    add_otp_task(user_id, task)


async def auto_otp_a2(user_id, numbers, display_range, bot, chat_id):
    """A2 OTP auto-poll — 10 sec interval, max 10 min."""
    TIMEOUT = 600
    start = time.time()
    received = set()
    ...
    while True:
        ...
        if time.time() - start > TIMEOUT:
            remaining = [n for n in numbers if n not in received]
            if remaining:
                try:
                    await bot.send_message(
                        chat_id,
                        "⏰ *A2 OTP Timeout*\n\n10 মিনিটে OTP পাওয়া যায়নি।\nনতুন number নিন।",
                        parse_mode="Markdown"
                    )
                except Exception:
                    pass
        ...
        for number in list(numbers):
            if number in received:
                continue
            try:
                entry = await a2_poll_otp(number)
                if not entry:
                    continue
                ...
                detected_app = detect_app_from_message(
                    message_text, user_data.get(user_id, {}).get("app", "FACEBOOK"))
                ...
                header = f"{flag} {country_name} {detected_app.capitalize()} (A2) → 🟢"
                inbox_text = f"{header}\n📱 Phone : {number}"
                btn_row = []
                if message_text:
                    btn_row.append(InlineKeyboardButton(
                        "📋 Full SMS",
                        copy_text=CopyTextButton(text=message_text[:256]),
                ...
                logger.info(f"A2 OTP sent → user {user_id}: {otp_code} for {number}")

            except Exception as e:
                logger.error(f"auto_otp_a2 error for {number}: {e}")
...
async def job_a2_range_post(context):
    ...
    try:
        ...
        # Build range → latest SMS map
        range_sms_map = {}
        for hit in (console_hits or []):
            h_range = hit.get("range", "").upper().replace("XXX", "").strip()
        ...
        for r in ranges:
            ...
            # Real SMS from console
            raw_sms = range_sms_map.get(clean_base, "")
            otp = extract_otp(raw_sms) if raw_sms else ""
            if not otp:
            ...
            text = (
                ...
                f"📱 Number : {escape_mdv2(display_range)}\n"
                f"🔐 Code : {escape_mdv2(otp)}"
            )
            if clean_sms:
                text += f"\n\n{escape_mdv2(clean_sms[:150])}"
...
async def panel_select_inline():
    ...
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("⚡ Facebook A2 (VOLTX)", callback_data="select_panel_A2_fb", api_kwargs={"style": "success"})],
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "select_panel_A2_fb":
        ...
        for r in ranges:
            ...
            clean_base = rng.upper().replace("XXX", "").replace("X", "").strip()
            code  = clean_base[:3] if len(clean_base) >= 3 else clean_base[:2]
            flag  = COUNTRY_FLAGS_CODE.get(code, COUNTRY_FLAGS_CODE.get(code[:2], "🌍"))
            cname = COUNTRY_NAMES_CODE.get(code, COUNTRY_NAMES_CODE.get(code[:2], code))
            key = cname
            if key not in country_map:
                country_map[key] = {"flag": flag, "name": cname, "ranges": []}
            country_map[key]["ranges"].append(r)
        buttons = []
        for cname, cdata in list(country_map.items())[:20]:
            count = len(cdata["ranges"])
            buttons.append([InlineKeyboardButton(
                f"{cdata['flag']} {cdata['name']} ({count})",
                callback_data=f"a2_country:{cname}",
                api_kwargs={"style": "primary"}
            )])
    ...
    # ── A2 Range select → Get numbers ──
    # ── A2 Country select → show ranges ──
    if data.startswith("a2_country:"):
        cname_sel = data.split("a2_country:", 1)[1]
        await safe_edit(query, "⏳ লোড হচ্ছে...")
        ranges = await a2_get_active_ranges()
        buttons = []
        for r in ranges:
...
async def post_init(app):
    ...
    logger.info("✅ 5-Panel Bot started! S1 + S2 + S3 + A1 + A2 active.")
...
def main():
    ...
    logger.info("✅ 5-Panel Bot running! S1 (StexSMS) + S2 (X.Mint) + S3 (CR API) + A1 (ZENEX) + A2 (VOLTX)")
