    ...
    async with pool.acquire() as conn:
        await conn.execute("""
            CREATE TABLE IF NOT EXISTS users (
                ...
                name TEXT,
                joined TEXT,
                app TEXT DEFAULT 'FACEBOOK',
                panel TEXT DEFAULT 'S1',
                country TEXT,
                range TEXT,
                last_number TEXT
            )
        """)
        await conn.execute("""
            CREATE TABLE IF NOT EXISTS posted_sms (
                unique_id TEXT PRIMARY KEY,
                created_at TIMESTAMP DEFAULT NOW()
            )
        """)
    logging.warning("✅ DB tables ready")

async def db_save_user_async(user_id, data: dict):
    """User data DB তে save করো (upsert)"""
    try:
        pool = await get_db()
        async with pool.acquire() as conn:
            await conn.execute("""
                INSERT INTO users (user_id, name, joined, app, panel, country, range, last_number)
            ...
            user_id,
            data.get("name", "User"),
            data.get("joined", datetime.now().strftime("%Y-%m-%d %H:%M")),
            data.get("app", "FACEBOOK"),
            data.get("panel", "S1"),
            data.get("country"),
            data.get("range"),
    ...
    try:
        ...
        async with pool.acquire() as conn:
            row = await conn.fetchrow(
                "SELECT unique_id FROM posted_sms WHERE unique_id = $1", unique_id
            )
...
async def db_mark_posted(unique_id: str):
    ...
    try:
        ...
        async with pool.acquire() as conn:
            await conn.execute(
                "INSERT INTO posted_sms (unique_id) VALUES ($1) ON CONFLICT DO NOTHING",
                unique_id
            )
    except Exception as e:
...
async def job_post_live_sms(context):
    ...
    try:
        ...
        for panel_label, logs in [("S1", s1_logs), ("S2", s2_logs)]:
            for log in logs:
                ...
                if unique_id in _posted_sms_ids:
                    continue
                if await db_is_posted(unique_id):
                    _posted_sms_ids.add(unique_id)
                    continue

                await db_mark_posted(unique_id)
                _posted_sms_ids.add(unique_id)  # memory cache ও রাখো (fast check)
        ...
        if messages_to_send:
            combined_text = "\n\n".join([m[0] for m in messages_to_send])
            keyboard = messages_to_send[0][1]
            await send_or_edit_post(bot, combined_text, keyboard)
...
async def post_init(application):
        ...
        async with pool.acquire() as conn:
            ...
            for u in rows:
                ...
                if uid not in user_data:
                    user_data[uid] = {}
                user_data[uid].setdefault("name", u["name"] or "User")
                user_data[uid].setdefault("joined", u["joined"] or "")
                user_data[uid].setdefault("app", u["app"] or "FACEBOOK")
                user_data[uid].setdefault("panel", u["panel"] or "S1")
                user_data[uid].setdefault("country", u["country"])
                user_data[uid].setdefault("range", u["range"])
                user_data[uid].setdefault("last_number", u["last_number"])
                user_data[uid].setdefault("waiting_for", None)
            logging.warning(f"✅ DB: {len(rows)} users loaded")
    except Exception as e:
    ...
    try:
        pool = await get_db()
        async with pool.acquire() as conn:
            rows2 = await conn.fetch("SELECT unique_id FROM posted_sms")
            for row in rows2:
                _posted_sms_ids.add(row["unique_id"])
            logging.warning(f"✅ DB: {len(rows2)} posted_sms loaded")
    except Exception as e:
