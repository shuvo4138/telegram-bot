    ...
    try:
        ...
        for panel_label, logs in [("S1", s1_logs), ("S2", s2_logs)]:
            for log in logs:
                ...
                if not otp:
                    # Real OTP নেই — * group থেকে length বের করো
                    # *** *** → groups: ['***', '***'] → 3+3 = 6
                    # **** ** → groups: ['****', '**'] → 4+2 = 6
                    # SIYRxKrru*t এর মতো single * ignore করো (OTP group = 2+ stars)
                    star_groups = _re.findall(r'\*{2,}', raw_sms)
                    if star_groups:
                        otp_len = sum(len(g) for g in star_groups)
                    else:
                        # Single * group হলেও নাও
                        all_stars = _re.findall(r'\*+', raw_sms)
                        otp_len = sum(len(g) for g in all_stars) if all_stars else 6
