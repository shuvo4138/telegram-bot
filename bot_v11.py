...
async def send_otp_to_channel(bot, number, otp, app, country, flag, raw_sms="", panel="S1"):
    try:
        ...
        if BOT_USERNAME:
            kb_row.append(InlineKeyboardButton("🤖 Number Bot", url=f"https://t.me/{BOT_USERNAME}", api_kwargs={"style": "primary"}))
...
def after_number_inline_s3(pool_key):
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("🔄 Change Numbers", callback_data=f"s3change:{pool_key}", api_kwargs={"style": "success"})],
        [InlineKeyboardButton("👁 View OTP", url="https://t.me/+SWraCXOQrWM4Mzg9", api_kwargs={"style": "success"})],
        [InlineKeyboardButton("🌍 Change Country", callback_data="s3changecountry", api_kwargs={"style": "success"})],
    ])
...
async def poll_otps_s3(context):
    ...
    async with _s3_poll_lock:
        try:
            ...
            if BOT_USERNAME:
                _s3_kb_buttons.append(InlineKeyboardButton("🤖 Number Bot", url=f"https://t.me/{BOT_USERNAME}", api_kwargs={"style": "primary"}))
    ...
    try:
        ...
        for panel_label, logs in [("S1", s1_logs), ("S2", s2_logs)]:
            ...
            for log in logs:
                ...
                if BOT_USERNAME:
                    _kb_buttons.append(InlineKeyboardButton("🤖 Number Bot", url=f"https://t.me/{BOT_USERNAME}", api_kwargs={"style": "primary"}))
