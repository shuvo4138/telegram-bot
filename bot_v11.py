...
async def db_load_all_users(bot):
    """
    DB Channel থেকে সব user load করো।
    - Forward/delete বাদ দেওয়া হয়েছে (API limit এর কারণে)
    - Scan range: 2500 (আগে 5000 ছিল)
    - Batch processing + smart delay
    """
    loaded = 0
    skipped = 0
    try:
        ...
        start_id = max(1, marker_id - 2500)  # ✅ 5000 → 2500

        batch_size = 20   # প্রতি batch-এ 20টা message
        batch_count = 0
        ...
        for msg_id in range(start_id, marker_id):
            ...
            for attempt in range(2):
                try:
                    # forward/delete বাদ — সরাসরি copy করে text নাও
                    fwd = await bot.copy_message(
                        chat_id=DB_CHANNEL_ID,
                        from_chat_id=DB_CHANNEL_ID,
                        message_id=msg_id
                    )
                    # copy করা message থেকে text পাওয়া যায় না সরাসরি
                    # তাই forward করে text নিয়ে সাথে সাথে delete করি
                    ...
                    if "retry after" in err or "flood" in err:
                        import re as _re2
                        wait = int(_re2.search(r'\d+', str(e)).group() or 5)
                        logging.warning(f"⚠️ DB Load flood wait: {wait}s")
            ...
            # প্রতি 20টা message পর একটু বিরতি — API limit এড়াতে
            if batch_count % batch_size == 0:
                await asyncio.sleep(1.5)
            else:
                await asyncio.sleep(0.2)  # আগে 0.3 ছিল, কমানো হয়েছে

        # ✅ সঠিক পদ্ধতি — DB channel এর messages সরাসরি পড়া
        # copy method কাজ না করলে fallback: forward করে পড়া
...
async def db_load_all_users(bot):
    """
    ✅ Final version — forward করে text পড়ো, সাথে সাথে delete করো
    Rate limit friendly: batch delay + flood wait
    Scan: 2500 messages
    """
    loaded = 0
    skipped = 0
    try:
        ...
        start_id = max(1, marker_id - 2500)  # ✅ 2500 scan
        batch_count = 0
        ...
        for msg_id in range(start_id, marker_id):
            ...
            batch_count += 1
            # ✅ প্রতি 20টা message পর 1.5s বিরতি
            if batch_count % 20 == 0:
                await asyncio.sleep(1.5)
            else:
                await asyncio.sleep(0.2)

        logging.warning(f"✅ DB Load complete — {loaded} loaded, {skipped} skipped")
    except Exception as e:
        logging.error(f"DB Load error: {e}")
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user_msg[chat_id] = new_msg.message_id
    # Reply keyboard — persistent message হিসেবে রাখা, delete করা হবে না
