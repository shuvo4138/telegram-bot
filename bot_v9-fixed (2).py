...
SESSION_POOL_SIZE = 100  # Back to 100 (50/50 split)
NUMBER_GET_SLOTS = 50
OTP_CHECK_SLOTS = 50
...
class XMintSessionPool:
    ...
    async def initialize(self):
        async with self.lock:
            ...
            tasks = [self._login_once() for _ in range(100)]  # Back to 100
            results = await asyncio.gather(*tasks, return_exceptions=True)
            ...
            for r in results:
                if isinstance(r, dict) and r.get("token"):
                    self.all_sessions.append(r)
                    if number_count < 50:  # Back to 50
                        await self.number_sessions.put(r)
                        number_count += 1
                    elif otp_count < 50:  # Back to 50
                        await self.otp_sessions.put(r)
                        otp_count += 1
