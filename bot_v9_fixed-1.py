...
class S2SessionPool:
    ...
    async def _warmup(self):
        ...
        async with self.lock:
            ...
            # Background auto-refresh শুরু করো
            asyncio.create_task(self._background_refresh())
            logger.info("🔄 S2 background session refresh started")

    async def _background_refresh(self):
        """Background task — session expire হওয়ার আগেই নতুন session ready রাখে।"""
        await asyncio.sleep(30)  # warmup শেষ হওয়ার পর শুরু
        while True:
            try:
                await asyncio.sleep(60)  # প্রতি ৬০ সেকেন্ডে চেক
                if not self.initialized:
                    continue

                now = time.time()
                refresh_threshold = self.TOKEN_TTL - 300  # expire হওয়ার ৫ মিনিট আগে

                sessions_to_refresh = [
                    s for s in self.all_sessions
                    if now - s.get("time", 0) > refresh_threshold and not self._is_disabled(s)
                ]

                for old_session in sessions_to_refresh:
                    new_s = await self._login_once()
                    if new_s.get("token"):
                        # all_sessions এ replace করো
                        try:
                            idx = self.all_sessions.index(old_session)
                            self.all_sessions[idx] = new_s
                        except ValueError:
                            self.all_sessions.append(new_s)
                        logger.info(f"🔄 S2 session pre-refreshed | old={old_session.get('token','')[:10]}... → new={new_s['token'][:10]}...")
                    await asyncio.sleep(random.uniform(3, 8))  # flood এড়াতে

            except Exception as e:
                logger.error(f"S2 background_refresh error: {e}")
    ...
    async def _get_valid_session(self, queue, pool_name):
        ...
        for _ in range(queue.qsize() + 1):
            ...
            if time.time() - session.get("time", 0) > self.TOKEN_TTL:
                # Token expired — Supabase থেকে delete + fresh login
                asyncio.create_task(_s2_delete_token_cache())
                new_s = await self._login_once()
                if new_s.get("token"):
                    asyncio.create_task(_s2_save_token_cache(new_s["token"], new_s.get("session", "")))
                    session = new_s
                else:
    ...
    async def _recover_session(self, session, queue, delay=60):
...
async def _poll_shark_otps(context):
    ...
    try:
        ...
        for row in shark_rows:
            ...
            # Channel post — same format as S3 but [Shark]
            quoted_sms = "\n".join(
                f">{escape_mdv2(line)}" for line in message.splitlines() if line.strip()
            )
            if otp_code:
                channel_msg = (
                    f"🆕 NEW OTP — {escape_mdv2(detected_app_cap)} \\[Shark\\]\n\n"
                ...
                channel_msg = (
                    f"📩 NEW SMS — {escape_mdv2(detected_app_cap)} \\[Shark\\]\n\n"
                    f"{flag} {escape_mdv2(country_name)}\n\n"
                    f"📱 `\\+{escape_mdv2(hidden)}`\n\n"
                    f"━━━━━━━━━━━━━━━\n📩 SMS:\n{quoted_sms}"
                )
            ...
            if not ch_sent:
                plain_msg = (
                    f"🆕 NEW OTP — {detected_app_cap} [Shark]\n\n"
                    f"{flag} {country_name}\n\n"
                    f"📱 +{hidden}\n🔑 {otp_code or 'N/A'}\n\n"
                    f"📩 SMS:\n{message[:300]}"
                )
