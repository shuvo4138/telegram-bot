...
class XMintSessionPool:
    ...
    async def initialize(self):
        async with self.lock:
            ...
            results = []
            for i in range(30):  # S2: 15 number + 15 OTP
                r = await self._login_once()
                results.append(r)
    ...
    async def _login_once(self):
        # ✅ Hardcoded token use করো — no login needed
        if XMINT_HARDCODED_TOKEN:
            return {
                "token": XMINT_HARDCODED_TOKEN,
    ...
    async def get_number_session(self):
        try:
            session = await asyncio.wait_for(self.number_sessions.get(), timeout=30)
            # ✅ Expire হলে re-login না, session reuse করো
            if time.time() - session.get("time", 0) > 1500:
                session["time"] = time.time()
            return session
        except asyncio.TimeoutError:
            # Queue খালি হলে existing session থেকে নাও
...
async def api_get_number_s2(range_val, app_name="FACEBOOK"):
    ...
    try:
        ...
        if res.status_code == 403:
            logging.warning("⚠️ S2 number: 403 — session expired, relogin")
...
async def api_get_info_s2(search="", status="", saved_session=None):
    """X.Mint থেকে OTP info নাও — ONLY saved_session"""
    ...
    try:
        ...
        if res.status_code == 403:
            logging.warning("⚠️ S2 OTP: 403 — session expired")
...
async def get_xmint_console_logs(force=False):
    ...
    try:
        ...
        headers = {
            ...
            'Content-Type': "application/json",
            'mauthtoken': token,
            'Cookie': f"mautToken={token}; session_token={sess}"
        }
        async with httpx.AsyncClient(timeout=15) as client:
            res = await client.get(
