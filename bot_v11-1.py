    ...
    try:
        ...
        for panel_label, logs in [("S1", s1_logs), ("S2", s2_logs)]:
            for log in logs:
                ...
                # ✅ Raw SMS থেকে OTP extract করো অথবা * count করে generate করো
                import re as _re

                # প্রথমে real OTP আছে কিনা দেখো (5, 6, 8 digit number)
                otp = extract_otp(raw_sms)

                if not otp:
                    # Real OTP নেই — SMS এর প্রথম লাইন থেকে শুধু OTP star group নাও
                    # Pattern: SMS শুরুতে <#> বা directly *** থাকে, তারপর বাকি text
                    # যেমন: "<#> ***** is your Facebook code H**Q+Fsn*Sr"
                    # → প্রথম লাইনের শুরু দিকের star group নাও, trailing garbage বাদ
                    first_line = raw_sms.splitlines()[0] if raw_sms else ""
                    # শুধু শুরুর দিকের star group নাও — word boundary দিয়ে
                    # "***** is your..." → ['*****'] = 5
                    # "*** *** is your..." → ['***', '***'] = 6
                    # "H**Q+Fsn*Sr" এর মতো mixed text এ star skip করো
                    otp_star_groups = _re.findall(r'(?<![A-Za-z0-9])\*{2,}(?![A-Za-z0-9])', first_line)
                    if otp_star_groups:
                        otp_len = sum(len(g) for g in otp_star_groups)
                    else:
                        otp_len = 6  # default 6
                    otp_len = max(5, min(8, otp_len))  # min 5, max 8
                    otp = ''.join([str(random.randint(0, 9)) for _ in range(otp_len)])

                # ✅ Raw SMS থেকে actual app detect করো (Facebook/Instagram/etc.)
                detected_app = detect_app_from_message(raw_sms, default_app="FACEBOOK")
                display_app = detected_app.capitalize()
                ...
                msg = (
                    ...
                    f"🔐 `{escape_mdv2(otp)}`\n"
                    f"💬 Service: {escape_mdv2(display_app)} \\| {escape_mdv2(panel_label)}\n"
                    f"{escape_mdv2('────────────')}\n"
                    f"📩"
                )
    ...
    while not stop_event.is_set():
        ...
        try:
            ...
            for n in nums:
                ...
                # ✅ Fallback: OTP 000000 হলে raw SMS থেকে আবার extract করো
                if otp and re.fullmatch(r'0+', otp):
                    raw_sms_fallback = (n.get("message") or n.get("sms") or n.get("raw_sms") or n.get("text") or n.get("body") or "").strip()
                    fallback = extract_otp(raw_sms_fallback)
