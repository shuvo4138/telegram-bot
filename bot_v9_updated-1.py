...
MINI_APP_URL = os.getenv("MINI_APP_URL", "").strip()      # e.g. https://xxx.railway.app
...
class S2SessionPool:
    ...
    async def initialize(self):
        async with self.lock:
            ...
            # Deploy এ Supabase থেকে cached token নাও (1টা)
    ...
    async def get_number_session(self):
        try:
            ...
            if time.time() - session.get("time", 0) > self.TOKEN_TTL:
                # Token expire — Supabase delete + fresh login + Supabase save
                asyncio.create_task(_s2_delete_token_cache())
                new_session = await self._login_once()
                if new_session.get("token"):
                    asyncio.create_task(_s2_save_token_cache(new_session["token"], new_session.get("session", "")))
                    return new_session
                session["time"] = time.time()
            return session
        except asyncio.TimeoutError:
...
def admin_keyboard_s1s2():
    return InlineKeyboardMarkup([
        ...
        # ── Sub Panels ──
        [
            InlineKeyboardButton("🔵 S1 Panel",   callback_data="admin_apistatus",     api_kwargs={"style": "primary"}),
            InlineKeyboardButton("🟢 S2 Panel",   callback_data="admin_apistatus",     api_kwargs={"style": "primary"}),
        ],
        # ── Bulk Control ──
        [
...
def admin_keyboard_s3():
    return InlineKeyboardMarkup([
        ...
        [
            ...
            InlineKeyboardButton("➕ Upload",      callback_data="s3admin_addnumbers", api_kwargs={"style": "success"}),
        ],
        # ── Danger ──
        [
...
def after_number_inline_s1s2(numbers, range_val, otp_status=None):
    ...
    otp_status = dict {number: True/False} — True মানে OTP এসেছে
    """
    if isinstance(numbers, str):
        numbers = [numbers]
    ...
    buttons.append([InlineKeyboardButton("🔄 New Number", callback_data=f"new_number_{range_val}", api_kwargs={"style": "success"})])
    buttons.append([InlineKeyboardButton("🌍 Change Region", callback_data="back_app", api_kwargs={"style": "success"})])
    buttons.append([InlineKeyboardButton("📢 Check OTP (Channel)", url=_ch_link, api_kwargs={"style": "primary"})])
    return InlineKeyboardMarkup(buttons)
...
async def auto_otp_multi(message, numbers, user_id, range_val, bot=None):
    ...
    def build_otp_msg():
        ...
        for i, num in enumerate(numbers):
            clean = str(num).replace("+", "").strip()
            st = num_state.get(clean, {})
            otps = st.get("otp_lines", [])
            circle = CIRCLE.get(i, f"[{i+1}]")
            lines.append(f"{circle} <code>{clean}</code>")
            if otps:
                for o in otps:
                    lines.append(f"✅ OTP: <code>{o}</code>")
            elif st.get("done"):
            ...
            for uid in matched_users:
                try:
                    ...
                    if not is_shark_pool(pool_key):
                        continue
                    inbox_label = get_short_label(pool_key) if pool_key else f"{flag} {detected_app_cap}"
                    time_only = dt.split(" ")[-1] if " " in dt else dt

                    session_numbers = session.get("numbers", []) if session else []
                    if not session_numbers and session and session.get("number"):
                        session_numbers = [session["number"]]
                    if len(session_numbers) > 1 and number in session_numbers:
                        num_index = session_numbers.index(number) + 1
                        num_label = f"Number {num_index}"
                    else:
                        num_label = "Number"
                    ...
                    await _s3_send_with_retry(
                        ...
                        parse_mode="Markdown", reply_markup=inbox_kb
                    )
                except Exception as e:
                ...
                try:
                    ...
                    for uid in matched_users:
                        try:
                            ...
                            # কোন number এর OTP সেটা clearly দেখাও
                            session_numbers = session.get("numbers", []) if session else []
                            if not session_numbers and session and session.get("number"):
                                session_numbers = [session["number"]]
                            if len(session_numbers) > 1 and number in session_numbers:
...
async def cmd_admin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = (
        ...
        f"🔵 S1: `{s1_pool.number_sessions.qsize()}` sessions\n"
        f"🟢 S2: `{s2_pool.number_sessions.qsize()}` sessions\n"
        f"🔴 S3: `{total_s3}` numbers\n\n"
        "━━━━━━━━━━━━━━━━━━"
    )
    await update.message.reply_text(msg, parse_mode="Markdown", reply_markup=admin_keyboard_s1s2())
    await update.message.reply_text(
        "🔴 *S3 Panel — CR API*",
        parse_mode="Markdown",
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("s3getcountry:"):
        ...
        for n in selected:
            asyncio.create_task(mark_number_assigned(n, user_id))
...
async def post_init(app):
    ...
    # Mini App web server + menu button
    asyncio.create_task(_run_mini_app_server())
    await _set_menu_button(app.bot)
...
<head>
  ...
  <title>NUMBER PANEL OTP</title>
  <script src="https://telegram.org/js/telegram-web-app.js"></script>
  <style>
    @import url('https://fonts.googleapis.com/css2?family=Exo+2:wght@400;600;700;800&family=JetBrains+Mono:wght@400;600&display=swap');
    :root {
      --g1:#00ff88;--g2:#00e676;--g3:#00c853;--g4:#069c50;--g5:#004d26;
      --bg:#050d0a;--card:#0a1a12;--txt:#e8fff4;--sub:#7ecda0;
      --glow:0 0 20px rgba(0,255,136,0.4);
    }
    *{margin:0;padding:0;box-sizing:border-box;-webkit-tap-highlight-color:transparent;}
    body{font-family:'Exo 2',sans-serif;background:var(--bg);min-height:100vh;overflow-x:hidden;position:relative;}
    ...
    @keyframes gridMove{100%{transform:translateY(40px);}}
    ...
    #loading-screen{position:fixed;inset:0;background:var(--bg);display:flex;flex-direction:column;
      align-items:center;justify-content:center;z-index:1000;transition:opacity .6s ease,visibility .6s ease;}
    #loading-screen.hidden{opacity:0;visibility:hidden;}
    .loader-ring{width:64px;height:64px;border:3px solid rgba(0,255,136,.1);border-top:3px solid var(--g1);
      border-radius:50%;animation:spin .8s linear infinite;box-shadow:var(--glow);}
    .loader-text{margin-top:20px;color:var(--g1);font-family:'JetBrains Mono',monospace;font-size:12px;
      letter-spacing:3px;animation:blink 1s ease-in-out infinite;}
    ...
    .header{text-align:center;padding:20px 0 28px;opacity:0;transform:translateY(-20px);
      animation:slideDown .6s ease .8s forwards;}
    .header-logo{width:64px;height:64px;background:linear-gradient(135deg,var(--g3),var(--g5));
      border-radius:20px;margin:0 auto 14px;display:flex;align-items:center;justify-content:center;
      font-size:28px;box-shadow:var(--glow),inset 0 1px 0 rgba(255,255,255,.1);position:relative;overflow:hidden;}
    .header-logo::after{content:'';position:absolute;top:-50%;left:-50%;width:200%;height:200%;
    ...
    .header-title{font-size:22px;font-weight:800;color:var(--txt);letter-spacing:1px;text-transform:uppercase;}
    .header-title span{background:linear-gradient(90deg,var(--g1),var(--g3));-webkit-background-clip:text;
      -webkit-text-fill-color:transparent;background-clip:text;}
    .header-sub{font-family:'JetBrains Mono',monospace;font-size:11px;color:var(--sub);letter-spacing:2px;margin-top:6px;}
    ...
    .status-bar{display:flex;align-items:center;justify-content:center;gap:8px;
      ...
      padding:8px 20px;margin-bottom:28px;opacity:0;animation:fadeIn .5s ease 1.1s forwards;}
    .status-dot{width:8px;height:8px;background:var(--g1);border-radius:50%;
      box-shadow:0 0 8px var(--g1);animation:pulse 2s ease-in-out infinite;}
    @keyframes pulse{0%,100%{box-shadow:0 0 8px var(--g1);transform:scale(1);}50%{box-shadow:0 0 16px var(--g1);transform:scale(1.2);}}
    .status-text{font-family:'JetBrains Mono',monospace;font-size:11px;color:var(--g2);letter-spacing:1px;}

    .btn-grid{display:flex;flex-direction:column;gap:14px;flex:1;}
    .panel-btn{position:relative;width:100%;padding:18px 24px;border:none;border-radius:18px;cursor:pointer;
      display:flex;align-items:center;gap:16px;overflow:hidden;
      transition:transform .15s ease,box-shadow .15s ease;opacity:0;transform:translateX(-30px);
      background:linear-gradient(135deg,var(--g4) 0%,#003d20 100%);
      border:1px solid rgba(0,255,136,.15);
      box-shadow:0 4px 20px rgba(0,0,0,.4),inset 0 1px 0 rgba(255,255,255,.06);}
    .panel-btn.animate-in{animation:slideInLeft .5s cubic-bezier(.34,1.56,.64,1) forwards;}
    .panel-btn:nth-child(1){animation-delay:1.0s;}
    .panel-btn:nth-child(2){animation-delay:1.15s;}
    .panel-btn:nth-child(3){animation-delay:1.30s;}
    .panel-btn:nth-child(4){animation-delay:1.45s;}
    .panel-btn:nth-child(5){animation-delay:1.60s;}
    .panel-btn::before{content:'';position:absolute;inset:0;
      background:linear-gradient(135deg,rgba(0,255,136,.15) 0%,transparent 60%);opacity:0;transition:opacity .3s ease;}
    .panel-btn::after{content:'';position:absolute;top:0;left:0;right:0;height:1px;
      background:linear-gradient(90deg,transparent,rgba(0,255,136,.4),transparent);}
    .panel-btn:active{transform:scale(.96) translateY(1px);box-shadow:0 2px 10px rgba(0,0,0,.4),var(--glow);}
    .panel-btn:active::before{opacity:1;}

    .btn-icon{width:48px;height:48px;border-radius:14px;background:rgba(0,255,136,.1);
      border:1px solid rgba(0,255,136,.2);display:flex;align-items:center;justify-content:center;
      font-size:22px;flex-shrink:0;transition:background .3s ease;}
    .panel-btn:active .btn-icon{background:rgba(0,255,136,.2);}
    .btn-content{flex:1;text-align:left;}
    .btn-label{font-size:16px;font-weight:700;color:var(--txt);letter-spacing:.5px;display:block;}
    .btn-desc{font-size:11px;color:var(--sub);font-family:'JetBrains Mono',monospace;
      letter-spacing:.5px;margin-top:3px;display:block;}
    .btn-arrow{color:rgba(0,255,136,.4);font-size:18px;transition:transform .2s ease,color .2s ease;}
    .panel-btn:active .btn-arrow{transform:translateX(4px);color:var(--g1);}

    .footer{text-align:center;padding-top:28px;opacity:0;animation:fadeIn .5s ease 2s forwards;}
    .footer-text{font-family:'JetBrains Mono',monospace;font-size:10px;color:rgba(126,205,160,.3);letter-spacing:2px;}

    @keyframes slideDown{to{opacity:1;transform:translateY(0);}}
    @keyframes slideInLeft{to{opacity:1;transform:translateX(0);}}
    @keyframes fadeIn{to{opacity:1;}}

    .ripple{position:absolute;border-radius:50%;background:rgba(0,255,136,.25);
...
<body>
  ...
  <div class="container">
    <div class="header">
      ...
      <div class="header-title">NUMBER <span>PANEL</span></div>
      <div class="header-sub">OTP • SMS • VERIFICATION</div>
    </div>
    ...
    <div class="btn-grid">
      <button class="panel-btn" onclick="sendCmd('/get_number',event)">
        <div class="btn-icon">📲</div>
        <div class="btn-content">
          <span class="btn-label">Get Number</span>
          <span class="btn-desc">Request a new virtual number</span>
        </div>
        <span class="btn-arrow">›</span>
      </button>
      <button class="panel-btn" onclick="sendCmd('/my_numbers',event)">
        <div class="btn-icon">📦</div>
        <div class="btn-content">
          <span class="btn-label">My Numbers</span>
          <span class="btn-desc">View your active numbers</span>
        </div>
        <span class="btn-arrow">›</span>
      </button>
      <button class="panel-btn" onclick="sendCmd('/custom_range',event)">
        <div class="btn-icon">🎯</div>
        <div class="btn-content">
          <span class="btn-label">Custom Range</span>
          <span class="btn-desc">Set a specific number range</span>
        </div>
        <span class="btn-arrow">›</span>
      </button>
      <button class="panel-btn" onclick="sendCmd('/live_traffic',event)">
        <div class="btn-icon">📡</div>
        <div class="btn-content">
          <span class="btn-label">Live Traffic</span>
          <span class="btn-desc">Monitor real-time OTP flow</span>
        </div>
        <span class="btn-arrow">›</span>
      </button>
      <button class="panel-btn" onclick="sendCmd('/support',event)">
        <div class="btn-icon">🛟</div>
        <div class="btn-content">
    ...
    <div class="footer">
      <div class="footer-text">NUMBER PANEL OTP © 2026</div>
    </div>
  </div>

  <script>
    const tg = window.Telegram?.WebApp;
    if(tg){tg.ready();tg.expand();tg.setHeaderColor('#050d0a');tg.setBackgroundColor('#050d0a');}

    window.addEventListener('load',()=>{
      setTimeout(()=>{
        document.getElementById('loading-screen').classList.add('hidden');
        document.querySelectorAll('.panel-btn').forEach(b=>b.classList.add('animate-in'));
      },1200);
    });

    function sendCmd(cmd,e){
      ...
      if(tg){tg.sendData(cmd);}else{console.log('CMD:',cmd);}
    }
  </script>
</body>
</html>"""
...
async def _mini_app_handler(request):
    return aiohttp_web.Response(text=MINI_APP_HTML, content_type="text/html")


async def _run_mini_app_server():
    """aiohttp দিয়ে Mini App serve করে — bot এর সাথে একই process এ চলে।"""
    if not MINI_APP_URL:
        logger.warning("⚠️ MINI_APP_URL not set — Mini App web server disabled.")
        return
    web_app = aiohttp_web.Application()
    web_app.router.add_get("/", _mini_app_handler)
    runner = aiohttp_web.AppRunner(web_app)
    await runner.setup()
    site = aiohttp_web.TCPSite(runner, "0.0.0.0", MINI_APP_PORT)
    await site.start()
    logger.info(f"🌐 Mini App server running on port {MINI_APP_PORT} | URL: {MINI_APP_URL}")


async def _set_menu_button(bot):
    """Bot এ Mini App menu button set করে।"""
    if not MINI_APP_URL:
        return
    try:
