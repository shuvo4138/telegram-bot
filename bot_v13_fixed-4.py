    ...
    try:
        ...
        for entry in otps:
            ...
            if assign_time_ms and entry_time > 0:
                # assign_time_ms থেকে ৫ সেকেন্ড আগের OTP ignore (clock drift buffer)
                if entry_time < (assign_time_ms - 5000):
            ...
            # ── Number match (doc: number = plain digits, no plus) ──
    ...
    while True:
        ...
        for idx, number in enumerate(list(numbers)):
            ...
            try:
                ...
                received.add(number)
                # otp_id খালি হলে message দিয়ে unique key বানাও
                _otp_uid = entry.get("otp_id") or f"{number}_{otp_code}_{message_text[:30]}"
