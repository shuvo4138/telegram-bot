...
async def api_get_number_s2(range_val, app_name="FACEBOOK"):
    """X.Mint থেকে number নাও - Session pool ব্যবহার করে (fast)"""
...
async def api_get_info_s2(search="", status=""):
    """X.Mint থেকে OTP info নাও - Session pool ব্যবহার করে (fast)"""
...
async def send_otp_to_channel(bot, number, otp, app, country, flag, raw_sms=""):
    ...
    try:
        ...
        if len(clean_num) > 4:
            hidden_num = clean_num[:3] + "*" * (len(clean_num) - 6) + clean_num[-3:]
        else:
            hidden_num = clean_num
        
        # ✨ নতুন ফরম্যাট - raw SMS সহ
        safe_raw_sms = raw_sms.replace("_", "\\_").replace("*", "\\*").replace("`", "\\`").replace("[", "\\[")
        msg = (
            f"✨ OTP received ✨\n\n"
            f"📞 Number : {hidden_num}\n"
        ...
        keyboard = InlineKeyboardMarkup([
            [
                InlineKeyboardButton("MAIN CHANNEL", url="https://t.me/alwaysrvice24hours"),
...
async def auto_otp_single(number, user_id, otp_found_event, result_holder):
    ...
    # Track already sent OTPs to avoid duplicate
    sent_otps = set()
    ...
    while not otp_found_event.is_set():
        ...
        if user_data[user_id].get("auto_otp_cancel") or otp_found_event.is_set():
            return
        try:
            # status filter ছাড়া — সব entry check করো (PostPaid সহ)
            if panel == "S1":
                data = await api_get_info(search=clean_num, status="")
            else:
                data = await api_get_info_s2(search=clean_num, status="")

            nums = []
            if data.get("meta", {}).get("code") == 200:
                nums = data["data"].get("numbers") or []

            # সব entries থেকে OTP collect করো — কোনো number/status filter নেই
            all_found = []
            ...
            for n in nums:
                ...
                if clean_num in api_num or api_num in clean_num:
                    ...
                    # OTP থাকলেই নাও — status যাই হোক (PostPaid, Safaricom, যেকোনো)
                    if otp and otp not in sent_otps:
                        # Country clean করো — carrier নাম হলে user এর country নাও
                        raw_country = n.get("country", "").strip()
                        carrier_keywords = ["postpaid", "prepaid", "safaricom", "orange", "mtn", "airtel", "vodacom", "glo", "9mobile", "tigo", "moov", "telecom"]
                        if raw_country.lower() in carrier_keywords or not raw_country:
                            clean_country = user_data[user_id].get("country", raw_country) or raw_country
                        else:
                            clean_country = raw_country
                        all_found.append({
                            "otp": otp,
                            "number": n.get("number", number),
                            "raw": raw_otp,
                            "country": clean_country,
                            "app": detect_app_from_message(raw_otp, app)
                        })

            if all_found and not otp_found_event.is_set():
                # সব OTP list হিসেবে save করো
                for item in all_found:
                    sent_otps.add(item["otp"])

                # প্রথমটা main result হিসেবে রাখো
                first = all_found[0]
                result_holder["otp"] = first["otp"]
                result_holder["number"] = first["number"]
                result_holder["raw"] = first["raw"]
                result_holder["country"] = first["country"]
                result_holder["app"] = first["app"]
                # সব OTP list আলাদাভাবে রাখো
                result_holder["all_otps"] = all_found
                otp_found_event.set()
                return
...
async def auto_otp_multi(message, numbers, user_id, range_val, bot=None):
    ...
    if found_otp:
        ...
        all_otps = result_holder.get("all_otps", [])

        if len(all_otps) > 1:
            # একাধিক OTP এসেছে — সবগুলো দেখাও
            # প্রতিটা OTP channel এ পাঠাও
            if bot:
                for item in all_otps:
                    try:
                        item_flag = get_flag(item["country"])
                        await send_otp_to_channel(bot, str(item["number"]).replace("+", "").strip(), item["otp"], item["app"], item["country"], item_flag, item["raw"])
                    except Exception as e:
                        logging.error(f"❌ Channel send error: {e}")

            # User কে সব OTP দেখাও
            try:
                otp_lines = ""
                carrier_keywords = ["postpaid", "prepaid", "safaricom", "orange", "mtn", "airtel", "vodacom", "glo", "9mobile", "tigo", "moov", "telecom", "we"]
                for i, item in enumerate(all_otps, 1):
                    # country টা carrier হলে user এর selected country দেখাও
                    display_country = item["country"]
                    if not display_country or display_country.lower() in carrier_keywords:
                        display_country = user_data[user_id].get("country", "") or found_country
                    item_flag = get_flag(display_country)
                    otp_lines += f"🔑 OTP {i} : `{item['otp']}` ({display_country} {item_flag})\n"
