...
async def a2_fetch_all_otps():
    """VOLTX /success-otp — একবার call করে সব OTP list return করে।"""
    try:
        async with httpx.AsyncClient(timeout=15) as client:
            res = await client.get(
        ...
        logger.info(f"A2 success-otp fetched: {len(otps)} otps")
        return otps
    except Exception as e:
...
async def auto_otp_a2(user_id, numbers, rids, display_range, bot, chat_id):
    ...
    while True:
        ...
        # ── একবারই API call — সব number এর জন্য ──
        try:
            all_otps = await a2_fetch_all_otps()
        except Exception as e:
            logger.error(f"auto_otp_a2 fetch error: {e}")
