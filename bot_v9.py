...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "select_panel_A2_fb":
        ...
        if not ranges:
            ...
            return
        # Facebook service এর range আগে, তারপর বাকি সব — max 40, deduplicated
        fb_ranges = [r for r in ranges if r.get("service", "").upper() in ("FACEBOOK", "FB")]
        other_ranges = [r for r in ranges if r.get("service", "").upper() not in ("FACEBOOK", "FB")]
        sorted_ranges = fb_ranges + other_ranges
        buttons = []
        seen = set()
        for r in sorted_ranges:
            ...
            code = a2_extract_country_code(rng)
            flag = COUNTRY_FLAGS_CODE.get(code, "🌍")
            country_name = COUNTRY_NAMES_CODE.get(code, "")
            label = f"{flag} {country_name} {rng}" if country_name else f"{flag} {rng}"
            buttons.append([InlineKeyboardButton(
                label,
