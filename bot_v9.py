...
class S2SessionPool:
    ...
    """
    S2_NUM_TARGET  = 50   # number sessions (optimized for throughput)
    S2_OTP_TARGET  = 15   # otp sessions (faster OTP checking)
    S2_TOTAL       = S2_NUM_TARGET + S2_OTP_TARGET
    TOKEN_TTL      = 1200  # seconds (20 min)
    COOLDOWN       = 0.3   # seconds between reuse (reduced for speed)
    ...
    async def _apply_cooldown(self, session):
        ...
        wait = self.COOLDOWN - (time.time() - last)
        if wait > 0:
            await asyncio.sleep(wait)
    ...
    async def _warmup(self):
        ...
        async with self.lock:
            ...
            for i in range(self.S2_TOTAL - (1 if cached_token else 0)):
                ...
                # staggered random delay (1-2s) — flood protection while faster warmup
    ...
    async def _get_valid_session(self, queue, pool_name):
        ...
        try:
            session = await asyncio.wait_for(queue.get(), timeout=10)
            return session
        except asyncio.TimeoutError:
