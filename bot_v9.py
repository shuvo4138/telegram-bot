...
JOIN_CHANNEL_LINK = f"https://t.me/{JOIN_CHANNEL_USERNAME}" if JOIN_CHANNEL_USERNAME else ""
...
class S2SessionPool:
    ...
    async def initialize(self):
        async with self.lock:
            ...
            number_count = otp_count = 0
            for i in range(60):
                if number_count >= 25 and otp_count >= 25:
                    break
                r = await self._login_once()
                if isinstance(r, dict) and r.get("token"):
                    self.all_sessions.append(r)
                    if number_count < 25:
                        ...
                        await self.otp_sessions.put(r)
                        otp_count += 1
                # Real human-like delay: 3~7 seconds between each login
        ...
        for attempt in range(3):
            try:
                ...
                if res.status_code == 403:
                    logger.warning(f"S2 Login 403 (attempt {attempt+1}) — retrying...")
                    await asyncio.sleep(3)
                    continue
                if res.status_code != 200:
                    logger.error(f"S2 Login failed: HTTP {res.status_code} (attempt {attempt+1})")
                ...
                if data.get("meta", {}).get("code") == 200:
                    ...
                    logger.error(f"S2 Login bad response (attempt {attempt+1}): {data}")
                    await asyncio.sleep(2)
            except Exception as e:
                logger.error(f"S2 Login error (attempt {attempt+1}): {e}")
                await asyncio.sleep(2)
        return {}
...
async def send_otp_to_channel(bot, number, otp, app, country, flag, raw_sms="", panel="S1"):
    try:
        ...
        if OTP_CHANNEL_LINK and len(OTP_CHANNEL_LINK) > 10:
            kb_row.append(InlineKeyboardButton("📢 OTP Channel", url=OTP_CHANNEL_LINK))
        if BOT_USERNAME:
            kb_row.append(InlineKeyboardButton("🤖 Bot", url=f"https://t.me/{BOT_USERNAME}"))
...
async def auto_otp_multi(message, numbers, user_id, range_val, bot=None):
    ...
    async def on_otp(otp, country_result, flag_result, detected_app, raw_sms):
        # ✅ FIX: Duplicate OTP check — same OTP দুইবার দেখানো বন্ধ
        if otp in otp_lines:
            return
                ...
                if data.get("meta", {}).get("code") == 200:
                    ...
                    for n in nums:
                        ...
                        if otp and otp not in otp_lines:
                            country_result = n.get("country", "") or country_r
...
async def poll_otps_s3(context):
    try:
        ...
        # OTP channel-এ শুধু Bot link — main channel link দেওয়া হবে না
        otp_channel_keyboard = InlineKeyboardMarkup([[
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    welcome_text = (
        ...
        f"🤖 NUMBER PANEL BOT\n"
        f"━━━━━━━━━━━━━━━━━━━━\n\n"
        f"⚡ Super Fast OTP Service\n"
        f"🌍 Worldwide Numbers\n"
        f"🔒 100% Secure & Trusted\n"
        f"🕐 24/7 Active Service\n\n"
        f"━━━━━━━━━━━━━━━━━━━━\n"
        f"👇 নিচের Menu থেকে শুরু করুন!"
    )
...
async def cmd_stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"🟢 S2 Pool: {s2_pool.number_sessions.qsize()}/25 number sessions\n"
...
async def post_init(app):
    ...
    logger.info("📦 Loading S3 data from Telegram storage...")
...
def main():
    ...
    # S1/S2 Live SMS post — every 120 seconds (60 থেকে বাড়ানো হলো, overlap এড়াতে)
