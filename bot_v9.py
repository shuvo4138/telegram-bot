...
SESSION_POOL_SIZE = 60   # S1: 30 number + 30 OTP
NUMBER_GET_SLOTS = 30
OTP_CHECK_SLOTS = 30
    ...
    async def initialize(self):
        async with self.lock:
            ...
            for i in range(SESSION_POOL_SIZE):
                ...
                results.append(r)
                await asyncio.sleep(0.3)  # delay — rate limit এড়াতে
...
class XMintSessionPool:
    ...
    async def initialize(self):
        async with self.lock:
            ...
            results = []
            for i in range(60):  # S2: 30 number + 30 OTP
                r = await self._login_once()
                results.append(r)
                await asyncio.sleep(0.3)  # delay — rate limit এড়াতে
...
async def auto_otp_multi(message, numbers, user_id, range_val, bot=None):
    ...
    async def _run():
        ...
        if found_otp:
            ...
            # Channel এ সবসময় পাঠাও (backup হিসেবে)
            if bot:
                try:
            ...
            # User কে দেখাও শুধু যদি এটাই তার current number হয়
            current_num = str(user_data[user_id].get("last_number", "")).replace("+", "").strip()
            if current_num == clean_found_num:
                try:
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("app_s1_"):
        ...
        # ✅ 5 সেকেন্ড wait — পুরনো loop এর শেষ call শেষ হতে দাও
        await safe_edit(query, "⏳ নতুন number আসছে... (5s)")
