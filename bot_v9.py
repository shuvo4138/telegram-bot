...
async def panel_select_inline():
    ...
    s3_total = fb_count + ig_count
    rows = [
        [InlineKeyboardButton("📘 Facebook", callback_data="service:facebook", api_kwargs={"style": "primary"})],
        [InlineKeyboardButton("📸 Instagram", callback_data="service:instagram", api_kwargs={"style": "primary"})],
        [InlineKeyboardButton("💬 WhatsApp", callback_data="select_panel_WA", api_kwargs={"style": "primary"})],
    ]
    if s3_total > 0:
        rows.append([InlineKeyboardButton(f"🔴 Saved Numbers ({s3_total})", callback_data="service:s3", api_kwargs={"style": "danger"})])
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("service:"):
        ...
        if svc == "facebook":
            ...
            # সব source থেকে country collect করো
            country_map = {}  # country_name → {panels}

            # S2 countries
            s2_countries = await get_countries_for_app("FACEBOOK", panel="S2")
            ...
            if ZENEX_API_KEY:
                try:
                    a1_ranges = await zenex_get_active_ranges(service="Facebook New Account")
                    for r in a1_ranges:
                        rng = r.get("range", "")
                        code = rng[:2] if rng else ""
                        cname = COUNTRY_NAMES_CODE.get(code, "")
                        if cname and cname not in country_map:
                            country_map[cname] = []
            ...
            if A2_API_KEY:
                try:
                    a2_ranges = a2_get_cached_ranges() or await a2_get_active_ranges(force=True)
                    fb_a2 = [r for r in a2_ranges if r.get("service","").upper() in ("FACEBOOK","FB")]
                    for r in fb_a2:
            ...
            country_map = {}
            s2_ig = await get_instagram_countries(panel="S2")
            for c in (s2_ig or []):
                if c not in country_map: country_map[c] = []
                country_map[c].append("S2")
            ...
            if ZENEX_API_KEY:
                try:
                    a1_ig = await zenex_get_active_ranges(service="Instagram Account")
                    for r in a1_ig:
                        rng = r.get("range", "")
                        code = rng[:2] if rng else ""
                        cname = COUNTRY_NAMES_CODE.get(code, "")
    ...
    if data.startswith("fb_country:"):
        ...
        # S2 ranges
        s2_ranges = await get_all_ranges_for_country("FACEBOOK", cname, panel="S2")
        ...
        if A2_API_KEY:
            try:
                a2_all = a2_get_cached_ranges() or await a2_get_active_ranges(force=True)
                fb_a2 = [r for r in a2_all if r.get("service","").upper() in ("FACEBOOK","FB")]
                for r in fb_a2:
    ...
    if data.startswith("ig_country:"):
        ...
        range_buttons = []
        s2_ranges = await get_instagram_ranges(cname, panel="S2")
        for r in (s2_ranges or [])[:10]:
            rng = r if isinstance(r, str) else r.get("range","")
            if rng:
                range_buttons.append(InlineKeyboardButton(f"📞 {rng}", callback_data=f"auto_range:S2:{rng}", api_kwargs={"style": "primary"}))
    ...
    if data.startswith("select_panel_") and data not in ("select_panel_A2_fb", "select_panel_S2"):
        ...
        if panel == "S3":
            ...
            # WhatsApp — A1, A2, S1+S2 — unified source select
            app_name = "WHATSAPP"
            user_data[user_id]["app"] = app_name
            user_data[user_id]["panel"] = "S2"  # default, range select এ override হবে
            await safe_edit(query, "⏳ লোড হচ্ছে...")
            ...
            if A2_API_KEY:
                try:
                    ...
                    a2_ranges_wa = [r for r in a2_all if r.get("service", "").upper() in ("WHATSAPP", "WA")]
                except Exception:
                    a2_ranges_wa = []
            ...
            has_s2 = bool(country_list)
            has_a1 = bool(a1_ranges_wa)
            has_a2 = bool(a2_ranges_wa)

            if not has_s2 and not has_a1 and not has_a2:
                await safe_edit(query,
            ...
            # Show source selection — A1 / A2 / S1+S2
            wa_buttons = []
            if has_a1:
            ...
            if has_s2:
                wa_buttons.append([InlineKeyboardButton("📗 WhatsApp S2", callback_data="wa_source:s2", api_kwargs={"style": "success"})])
