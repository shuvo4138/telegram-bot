...
async def get_xmint_console_logs(force=False):
    ...
    try:
        ...
        if not token:
            return _xmint_console_cache["logs"]
        cookie = session.get("cookie", f"mauthtoken={token}")
        headers = {
            'authorization': token,
            'cookie': cookie,
    ...
    while not stop_event.is_set():
        ...
        try:
            ...
            if panel == "S1":
                ...
                # S2: প্রথমে status="success" চেষ্টা করো, না পেলে status="" দিয়ে চেষ্টা করো
...
async def cmd_mynum(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if panel == "S1":
        ...
        # S2 এ session দরকার — pool থেকে নাও, use করো, return করো
        _s2_session = await xmint_pool.get_otp_session()
        data = await api_get_info_s2(saved_session=_s2_session)
