...
#         MONGODB SETUP
# =============================================
MONGO_URI = os.environ.get("MONGO_URI", "mongodb+srv://shovosrb_db_user:ikuxhExaCVDXobqe@botservice.yjvl5nm.mongodb.net/?appName=Botservice&tlsAllowInvalidCertificates=true")
_mongo_col = None
...
def get_db_col():
    ...
    if _mongo_col is None and MONGO_URI:
        try:
            ...
            return None
        try:
            _mongo_col = client["otp_bot"]["users"]
            logging.info("✅ MongoDB connected!")
        except Exception as e:
            logging.error(f"❌ MongoDB collection error: {e}")
    return _mongo_col

def db_load_all_users():
    """Bot start এ DB থেকে সব user data load করো"""
    col = get_db_col()
    if col is None:
        return {}
    try:
        ...
        for doc in col.find():
            ...
            if uid:
                ...
                result[uid] = d
        logging.info(f"✅ MongoDB: {len(result)} users loaded")
        return result
    except Exception as e:
        logging.error(f"❌ DB load error: {e}")
        return {}

def db_save_user(user_id, data):
    """একজন user এর data save করো"""
    col = get_db_col()
    if col is None:
        return
    try:
        safe = {k: v for k, v in data.items() if k not in (
            "number_session", "otp_running", "otp_active", "auto_otp_cancel",
            "waiting_for"
        )}
        col.update_one(
            {"user_id": user_id},
        ...
        logging.error(f"❌ DB save error (user {user_id}): {e}")
    ...
    async def initialize(self):
        async with self.lock:
            ...
            for i in range(50):  # S2: 25 number + 25 OTP
                r = await self._login_once()
                results.append(r)
                await asyncio.sleep(3)  # 5sec → 3sec
...
async def do_get_number(message, user_id, count=1, user_name="User", bot=None):
    ...
    if count == 1:
        ...
        if data.get("meta", {}).get("code") == 200:
            ...
            db_save_user(user_id, user_data[user_id])
...
async def post_init(application):
    try:
        # ✅ MongoDB থেকে user data load করো
        loaded = db_load_all_users()
        user_data.update(loaded)
        logging.info(f"✅ User data loaded from MongoDB: {len(loaded)} users")
    except Exception as e:
        logging.error(f"⚠️ MongoDB load error: {e}")
