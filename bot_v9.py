...
async def a2_fetch_all_otps(rid=None):
    """VOLTX /success-otp — POST + rid দিয়ে OTP list return করে।
    BUG FIX v16: আগে GET করত, কিন্তু API তে POST + {"rid": "..."} লাগে।
    rid দিলে সেই specific range এর OTP আনে।
    """
    try:
        body = {"rid": str(rid)} if rid else {}
        async with httpx.AsyncClient(timeout=15) as client:
            res = await client.post(
