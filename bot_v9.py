...
class S2SessionPool:
    ...
    async def _get_valid_session(self, queue, pool_name):
        ...
        try:
            session = await asyncio.wait_for(queue.get(), timeout=60)
            return session
        except asyncio.TimeoutError:
...
async def send_otp_to_channel(bot, number, otp, app, country, flag, raw_sms="", panel="S1"):
    try:
        ...
        if BOT_USERNAME:
            kb_row.append(InlineKeyboardButton("🤖 Number Bot", url=f"https://t.me/{BOT_USERNAME}", api_kwargs={"style": "success"}))
...
def after_number_inline_s1s2(number, range_val):
    buttons = [
        [InlineKeyboardButton("🔄 New Number", callback_data=f"new_number_{range_val}", api_kwargs={"style": "success"})],
        [InlineKeyboardButton("🌍 Change Region", callback_data="back_app", api_kwargs={"style": "success"})],
    ]
    _ch_link = "https://t.me/+SWraCXOQrWM4Mzg9"
    buttons.insert(1, [InlineKeyboardButton("📢 Check OTP (Channel)", url=_ch_link, api_kwargs={"style": "primary"})])
    return InlineKeyboardMarkup(buttons)
...
async def auto_otp_multi(message, numbers, user_id, range_val, bot=None):
    ...
    def build_waiting_msg():
        lines = [f"<blockquote>🌍 {flag} {country_r or "Unknown"} | {app.title()}</blockquote>"]
        for i, num in enumerate(numbers):
            clean = str(num).replace("+", "").strip()
            st = num_state.get(clean, {})
            if st.get("done") and st.get("otp_lines"):
                last_otp = st["otp_lines"][-1]
                lines.append(f'{CIRCLE.get(i, str(i+1))} +{clean}')
                lines.append(f'<code>{last_otp}</code> ✅')
            elif st.get("done"):
                lines.append(f'{CIRCLE.get(i, str(i+1))} +{clean} ⌛ Expired')
            else:
                lines.append(_number_line(i, clean))
        lines.append("")
        lines.append("⏳ <b>Waiting For OTP...</b>")
        return "\n".join(lines)

    def build_otp_msg():
        lines = [f"<blockquote>📩 OTP RECEIVED</blockquote>"]
        for i, num in enumerate(numbers):
            clean = str(num).replace("+", "").strip()
            st = num_state.get(clean, {})
    ...
    async def update_msg(text):
        ...
        kb = after_number_inline_s1s2(numbers[0], range_val)
        try:
            if sent_message:
                try:
        ...
        if chat_id in user_msg:
            try:
                sent_message = await bot.edit_message_text(
                    ...
                    text=build_waiting_msg(),
                    parse_mode="HTML",
                    reply_markup=after_number_inline_s1s2(numbers[0], range_val)
                )
            except Exception:
                sent_message = await message.reply_text(
                    build_waiting_msg(), parse_mode="HTML",
                    reply_markup=after_number_inline_s1s2(numbers[0], range_val)
                )
                user_msg[chat_id] = sent_message.message_id
        else:
...
async def poll_otps_s3(context):
    ...
    async with _s3_poll_lock:
        try:
            ...
            if BOT_USERNAME:
                _s3_kb_buttons.append(InlineKeyboardButton("🤖 Number Bot", url=f"https://t.me/{BOT_USERNAME}", api_kwargs={"style": "success"}))
    ...
    try:
        ...
        for panel_label, logs in [("S1", s1_logs), ("S2", s2_logs)]:
            ...
            for log in logs:
                ...
                if BOT_USERNAME:
                    _kb_buttons.append(InlineKeyboardButton("🤖 Number Bot", url=f"https://t.me/{BOT_USERNAME}", api_kwargs={"style": "success"}))
