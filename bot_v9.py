...
async def a2_poll_otp(target_number, after_ms=0):
    """VOLTX OTP poll — GET /success-otp, number + time দিয়ে filter।
    after_ms: unix milliseconds — এর আগের OTP ignore করো।
    """
    try:
        async with httpx.AsyncClient(timeout=15) as client:
            res = await client.get(
        ...
        if res.status_code == 200:
            data = res.json()
            logger.info(f"A2 success-otp raw: {res.text[:400]}")
            if data.get("meta", {}).get("code") == 200:
                otps = data.get("data", {}).get("otps", [])
                clean_target = str(target_number).lstrip("+").strip()
                for entry in otps:
                    num = str(entry.get("number", "")).lstrip("+").strip()
                    otp_time = entry.get("time", 0)
                    # number match + time check (after_ms=0 মানে সব নাও)
                    if (num == clean_target or num.endswith(clean_target[-7:])):
                        if after_ms == 0 or otp_time >= after_ms:
                            return entry
        return None
    except Exception as e:
...
async def auto_otp_a2(user_id, numbers, display_range, bot, chat_id):
    ...
    TIMEOUT = 600
    start = time.time()
    start_ms = int(start * 1000)  # number allocate এর পরের OTP ই নেওয়া হবে
    received = set()
    ...
    while True:
        ...
        for number in list(numbers):
            if number in received:
                continue
            try:
                entry = await a2_poll_otp(number, after_ms=start_ms)
                if not entry:
                    continue
