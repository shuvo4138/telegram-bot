        ...
        try:
            ...
            if time.time() - session.get("time", 0) > 1500:
                ...
                if new_session.get("token"):
                    return new_session
                # ✅ FIX: নতুন session না পেলে পুরানোটাই সময় update করে ফেরত দাও
...
async def auto_otp_multi(message, numbers, user_id, range_val, bot=None):
    ...
    async def _run():
        ...
        # ✅ FIX: panel অনুযায়ী সঠিক OTP function call করো
        def make_otp_task():
            return asyncio.create_task(auto_otp_single_s1(number, user_id, stop_event))

        inner_task = make_otp_task()
