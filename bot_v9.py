...
async def send_otp_to_channel(bot, number, otp, app, country, flag, raw_sms="", panel="S1"):
    try:
        header = _build_otp_post_header(number, app, country, flag, panel)
        keyboard = _build_unified_keyboard()

        # ── OTP copy box (green, full width) ──
        kb_rows = []
        if otp:
        ...
        _ch_link = OTP_CHANNEL_LINK or MAIN_CHANNEL_LINK or JOIN_CHANNEL_LINK or ""
        if _ch_link and len(_ch_link) > 10:
            kb_btm.append(InlineKeyboardButton("📢 Main Channel", url=_ch_link, api_kwargs={"style": "primary"}))
        if BOT_USERNAME:
            kb_btm.append(InlineKeyboardButton("🤖 Number Bot", url=f"https://t.me/{BOT_USERNAME}", api_kwargs={"style": "danger"}))
        if kb_btm:
            kb_rows.append(kb_btm)
        keyboard = InlineKeyboardMarkup(kb_rows) if kb_rows else None

        text = escape_mdv2(header)
        try:
            await safe_send_message(bot, OTP_CHANNEL_ID, text, parse_mode="MarkdownV2", reply_markup=keyboard)
        except Exception:
            await safe_send_message(bot, OTP_CHANNEL_ID, header, reply_markup=keyboard)
    except Exception as e:
...
async def _poll_shark_otps(context):
    ...
    try:
        ...
        for row in shark_rows:
            ...
            s3v1_header = _build_otp_post_header(number, detected_app_cap, country_name, flag, "S3v1")
            s3v1_text = escape_mdv2(s3v1_header)

            s3v1_kb_rows = []
            if otp_code:
                s3v1_kb_rows.append([InlineKeyboardButton(
                    f"{otp_code}",
                    copy_text=CopyTextButton(text=otp_code),
                    api_kwargs={"style": "success"}
                )])
            _s3v1_btm = []
            _s3v1_lnk = OTP_CHANNEL_LINK or MAIN_CHANNEL_LINK or JOIN_CHANNEL_LINK or ""
            if _s3v1_lnk and len(_s3v1_lnk) > 10:
                _s3v1_btm.append(InlineKeyboardButton("📢 Main Channel", url=_s3v1_lnk, api_kwargs={"style": "primary"}))
            if BOT_USERNAME:
                _s3v1_btm.append(InlineKeyboardButton("🤖 Number Bot", url=f"https://t.me/{BOT_USERNAME}", api_kwargs={"style": "danger"}))
            if _s3v1_btm:
                s3v1_kb_rows.append(_s3v1_btm)
            s3v1_keyboard = InlineKeyboardMarkup(s3v1_kb_rows) if s3v1_kb_rows else otp_channel_keyboard

            ch_sent = await _s3_send_with_retry(
                context.bot, OTP_CHANNEL_ID, s3v1_text,
                parse_mode="MarkdownV2", reply_markup=s3v1_keyboard
            )
            if not ch_sent:
                ch_sent = await _s3_send_with_retry(
                    context.bot, OTP_CHANNEL_ID, s3v1_header,
                    reply_markup=s3v1_keyboard
                )
            if ch_sent:
                sent_count += 1
        ...
        try:
            ...
            for otp_data in cr_otps:
                try:
                    ...
                    # Header: 🇬🇳 GN FB 2246***445 [S3]
                    s3_header = _build_otp_post_header(number, detected_app_cap, country_name, flag, "S3")
                    s3_text = escape_mdv2(s3_header)

                    # OTP copy box keyboard
                    s3_kb_rows = []
                    if otp_code:
                        s3_kb_rows.append([InlineKeyboardButton(
                            f"{otp_code}",
                            copy_text=CopyTextButton(text=otp_code),
                            api_kwargs={"style": "success"}
                        )])
                    _s3_btm = []
                    _s3_ch_lnk = OTP_CHANNEL_LINK or MAIN_CHANNEL_LINK or JOIN_CHANNEL_LINK or ""
                    if _s3_ch_lnk and len(_s3_ch_lnk) > 10:
                        _s3_btm.append(InlineKeyboardButton("📢 Main Channel", url=_s3_ch_lnk, api_kwargs={"style": "primary"}))
                    if BOT_USERNAME:
                        _s3_btm.append(InlineKeyboardButton("🤖 Number Bot", url=f"https://t.me/{BOT_USERNAME}", api_kwargs={"style": "danger"}))
                    if _s3_btm:
                        s3_kb_rows.append(_s3_btm)
                    s3_keyboard = InlineKeyboardMarkup(s3_kb_rows) if s3_kb_rows else otp_channel_keyboard

                    ch_sent = await _s3_send_with_retry(
                        context.bot, OTP_CHANNEL_ID, s3_text,
                        parse_mode="MarkdownV2", reply_markup=s3_keyboard
                    )
                    if not ch_sent:
                        ch_sent = await _s3_send_with_retry(
                            context.bot, OTP_CHANNEL_ID, s3_header,
                            reply_markup=s3_keyboard
                        )
    ...
    try:
        ...
        for panel_label, logs in [("S1", s1_logs), ("S2", s2_logs)]:
            ...
            for log in logs:
                ...
                svc_short = "WA" if app == "WHATSAPP" else "FB"

                # ── Header: 🇬🇳 Guinea FB 224655XXX [S1] ──
                header_line = f"{flag} {country} {svc_short} {display_range} [{panel_label}]"
                text = escape_mdv2(header_line)
                ...
                if BOT_USERNAME:
                    _kb_btm.append(InlineKeyboardButton("🤖 Number Bot", url=f"https://t.me/{BOT_USERNAME}", api_kwargs={"style": "danger"}))
                if _kb_btm:
...
async def job_a1_range_post(context):
    ...
    try:
        ...
        # একবার numsuccess/info ফেচ করি সব range এর জন্য
        zenex_otps = []
        try:
            async with httpx.AsyncClient(timeout=15) as client:
                res = await client.get(
            ...
            if res.status_code == 200:
                zenex_otps = res.json().get("data", {}).get("otps", [])
                logger.info(f"A1 numsuccess: {len(zenex_otps)} OTP entries")
        except Exception as e:
            logger.error(f"A1 numsuccess fetch error: {e}")
        ...
        for r in ranges:
            ...
            # raw SMS খোঁজো numsuccess/info থেকে
            clean_prefix = rng.upper().replace("XXX", "").replace("X", "").strip()
            raw_sms = ""
            otp = ""
            for entry in zenex_otps:
                num = str(entry.get("number", "")).lstrip("+")
                if num.startswith(clean_prefix):
                    raw_sms = (entry.get("message") or entry.get("sms") or "").strip()
                    otp = str(entry.get("otp") or "").strip()
                    break

            if not otp:
                # ── A1 service অনুযায়ী OTP length ──
                svc_lower = service.lower()
                if "instagram" in svc_lower:
                    _a1_otp_len = 6
                elif "pc clone" in svc_lower or "clone" in svc_lower:
                    _a1_otp_len = random.choice([6, 7, 8])
                elif "new account" in svc_lower or "facebook" in svc_lower:
                    _a1_otp_len = 5
                else:
                    _a1_otp_len = 6
                otp = ''.join([str(random.randint(0, 9)) for _ in range(_a1_otp_len)])
            # ── Range display: always ends with XXX ──
            clean_base = rng.upper().replace("XXX", "").replace("X", "").strip()
            display_range = clean_base + "XXX"

            # ── Country flag + country name + service short ──
            ...
            svc_short = "FB" if "facebook" in service.lower() else ("IG" if "instagram" in service.lower() else ("WA" if "whatsapp" in service.lower() else service[:2].upper()))

            # ── Header: e.g. 🇹🇿 Tanzania FB 2559743XXX [A1] ──
            header_line = f"{range_flag} {country_name} {svc_short} {display_range} [A1]"
            text = escape_mdv2(header_line)
            ...
            # Bottom: Main Channel (primary/blue) + Number Bot (danger/red)
            bottom_row = []
            ...
            if BOT_USERNAME:
                bottom_row.append(InlineKeyboardButton("🤖 Number Bot", url=f"https://t.me/{BOT_USERNAME}", api_kwargs={"style": "danger"}))
            if bottom_row:
    ...
    try:
        ...
        for panel_label, logs in [("S1", s1_logs), ("S2", s2_logs)]:
            ...
            for log in logs:
                ...
                # ── Header: 🇬🇳 Guinea WA 224655XXX [S1] ──
                clean_base_wa = display_range.replace("XXX","").replace("X","").strip()
                wa_header = f"{flag} {country} WA {display_range} [{panel_label}]"
                wa_text = escape_mdv2(wa_header)
                wa_kb_rows = []
                wa_kb_rows.append([InlineKeyboardButton(f"{otp}", copy_text=CopyTextButton(text=otp), api_kwargs={"style": "success"})])
                _wa_btm = []
                ...
                if BOT_USERNAME:
                    _wa_btm.append(InlineKeyboardButton("🤖 Number Bot", url=f"https://t.me/{BOT_USERNAME}", api_kwargs={"style": "danger"}))
                if _wa_btm:
        ...
        if post_count < WA_MAX and ZENEX_API_KEY:
            try:
                ...
                for r in a1_wa_ranges:
                    ...
                    wa_a1_base = rng.upper().replace("XXX","").replace("X","").strip()
                    display_range = wa_a1_base + "XXX"
                    wa_a1_code = wa_a1_base[:3] if len(wa_a1_base) >= 3 else wa_a1_base[:2]
                    wa_a1_flag = COUNTRY_FLAGS_CODE.get(wa_a1_code, COUNTRY_FLAGS_CODE.get(wa_a1_code[:2], "🌍"))
                    wa_a1_country = COUNTRY_NAMES_CODE.get(wa_a1_code, COUNTRY_NAMES_CODE.get(wa_a1_code[:2], wa_a1_code))
                    wa_a1_header = f"{wa_a1_flag} {wa_a1_country} WA {display_range} [A1]"
                    wa_a1_text = escape_mdv2(wa_a1_header)
                    wa_a1_kb = []
                    wa_a1_kb.append([InlineKeyboardButton(f"{otp}", copy_text=CopyTextButton(text=otp), api_kwargs={"style": "success"})])
                    _wa_a1_btm = []
                    _wa_a1_lnk = OTP_CHANNEL_LINK or MAIN_CHANNEL_LINK or JOIN_CHANNEL_LINK or ""
                    if _wa_a1_lnk and len(_wa_a1_lnk) > 10:
                        _wa_a1_btm.append(InlineKeyboardButton("📢 Main Channel", url=_wa_a1_lnk, api_kwargs={"style": "primary"}))
                    if BOT_USERNAME:
                        _wa_a1_btm.append(InlineKeyboardButton("🤖 Number Bot", url=f"https://t.me/{BOT_USERNAME}", api_kwargs={"style": "danger"}))
                    if _wa_a1_btm:
                        wa_a1_kb.append(_wa_a1_btm)
                    wa_a1_keyboard = InlineKeyboardMarkup(wa_a1_kb)
                    try:
                        await safe_send_message(bot, chat_id=RANGE_CHANNEL_ID, text=wa_a1_text, parse_mode="MarkdownV2", reply_markup=wa_a1_keyboard)
                        _posted_sms_ids.add(unique_id)
                        post_count += 1
                        await asyncio.sleep(5)
                    except Exception as e:
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("select_panel_"):
        ...
        if panel == "S3":
            ...
            # Show source selection — A1, S1, S2 আলাদা
            wa_buttons = []
            if has_a1:
                wa_buttons.append([InlineKeyboardButton("💬 A1 WhatsApp", callback_data="wa_source:a1", api_kwargs={"style": "success"})])
            if has_s1s2:
                wa_buttons.append([InlineKeyboardButton("💬 S1 WhatsApp", callback_data="wa_source:s1", api_kwargs={"style": "primary"})])
                wa_buttons.append([InlineKeyboardButton("💬 S2 WhatsApp", callback_data="wa_source:s2", api_kwargs={"style": "primary"})])
    ...
    if data.startswith("wa_source:"):
        ...
        elif source in ("s1", "s2"):
            # WhatsApp S1 বা S2
            panel_label = source.upper()
            user_data[user_id]["panel"] = panel_label
            await safe_edit(query, "⏳ লোড হচ্ছে...")
            countries = await get_countries_for_app(app_name, panel=panel_label)
            country_list = [{"country": c, "panel": panel_label} for c in (countries or [])]
            if not country_list:
                await safe_edit(query,
                    f"❌ WhatsApp {panel_label} তে এখন কোনো active range নেই।",
                    reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("◀️ Back", callback_data="select_panel_WA", api_kwargs={"style": "primary"})]])
                )
                return
            await safe_edit(query,
                f"💬 {panel_label} WhatsApp\n\n🌍 Country select করুন:",
                reply_markup=country_select_inline(country_list, app_name)
            )
        else:
            # fallback — s1s2 combined (backward compat)
