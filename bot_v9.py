...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("select_panel_") and data not in ("select_panel_A2_fb", "select_panel_S2"):
        ...
        if panel == "S3":
            ...
            if not has_s1s2 and not has_a1 and not has_a2:
                await safe_edit(query,
            ...
            if has_s1s2:
                wa_buttons.append([InlineKeyboardButton("📗 WhatsApp S1/S2", callback_data="wa_source:s1s2", api_kwargs={"style": "success"})])
