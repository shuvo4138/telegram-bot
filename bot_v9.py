    ...
    try:
        ...
        for panel_label, logs in [("S1", s1_logs), ("S2", s2_logs)]:
            for log in logs:
                ...
                # শুধু last 5 মিনিটের log নেবো
                from datetime import timezone, timedelta as _td
                now_bd = datetime.now(timezone(_td(hours=6)))
                log_time_str = log.get("time", "").strip()
                try:
                    log_dt = datetime.strptime(log_time_str, "%H:%M:%S").replace(
                        year=now_bd.year, month=now_bd.month, day=now_bd.day,
                        tzinfo=timezone(_td(hours=6))
                    )
                    if (now_bd - log_dt).total_seconds() > 300:
                        continue
                except Exception:
                    pass
