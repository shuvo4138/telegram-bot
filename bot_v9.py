...
XMINT_API_URL = "https://x.mnitnetwork.com/api"

# ✅ Hardcoded token — login করতে হবে না
XMINT_HARDCODED_TOKEN = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1aWQiOiJNX1RWUTMyYU5ZSylsInJvbGUiOiJ1c2VyIiwiYWNjZXNzX3BhdGgiOlsiL2Rhc2hib2FyZCJdLCJleHBpcnkiOjE3NzQyODgyMzgsImNyZWF0ZWQiOjE3NzQyMDE4MzgsMm9yOiJNQ0giLCJleHAiOjE3NzQyODgyMzgsImlhdCI6MTc3NDIwMTgzOCwic3ViIjoiTV9UVlEzMllVNVpTeWxzIn0.5mhvrmPtHOCAUIWO1FHtkxizyVoElv7Tg-CIZvD8UEs"
...
# =============================================
#         X.MINT SESSION (S2) - Persistent Client
# =============================================
...
class XMintSession:
    ...
    async def login(self):
        """Fresh login with persistent client"""
        try:
            ...
            res = await self.client.post(
                f"{XMINT_API_URL}/login",
                json={"email": XMINT_EMAIL, "password": XMINT_PASSWORD}
            )
            logging.info(f"X.Mint login: {res.status_code}")
            if res.status_code == 200:
                self.logged_in = True
                logging.info("✅ X.Mint login success")
                return True
            logging.error(f"X.Mint login failed: {res.text[:100]}")
            return False
        except Exception as e:
    ...
    async def get(self, url, **kwargs):
        ...
        try:
            ...
            if res.status_code == 401:
                logging.warning("X.Mint 401 — relogin")
                self.logged_in = False
                await self.login()
                res = await self.client.get(url, **kwargs)
            return res
        except Exception as e:
            logging.error(f"X.Mint GET error: {e}")
            return None
    ...
    async def post(self, url, **kwargs):
        ...
        try:
            ...
            logging.error(f"X.Mint POST error: {e}")
            return None

xmint_session = XMintSession()

# Backward compatibility — dummy pool
class XMintSessionPool:
    ...
    async def initialize(self):
        async with self.lock:
            if self.initialized:
                return
            logging.info("🔄 X.Mint session initialize হচ্ছে...")
            success = await xmint_session.login()
            self.initialized = True
            if success:
                logging.info("✅ X.Mint session ready!")
            else:
                logging.error("❌ X.Mint login failed")

    async def get_number_session(self):
        return {"token": "xmint", "cookie": ""}
...
async def api_get_number_s2(range_val, app_name="FACEBOOK"):
    """X.Mint থেকে number নাও — persistent client"""
    ...
    if res.status_code == 403:
        return {"error": "Session expired"}, None

    try:
        result = res.json()
        # ✅ session হিসেবে dummy দিই — persistent client handle করবে
        session_ref = {"token": "xmint_persistent", "panel": "S2"}
        return result, session_ref
    except Exception as e:
        return {"error": str(e)}, None

async def api_get_info_s2(search="", status="", saved_session=None):
    """X.Mint থেকে OTP info নাও — persistent client"""
    clean_search = search.replace("+", "").strip()
    today = datetime.now().strftime("%Y-%m-%d")
    params = {"date": today, "page": 1, "search": clean_search, "status": status}

    res = await xmint_session.get(
...
async def send_otp_to_channel(bot, number, otp, app, country, flag, raw_sms=""):
    try:
        ...
        # App emoji
        app_emoji = APP_EMOJIS.get(app_cap, "📱")
        ...
        msg = (
            f"{escape_mdv2(app_emoji)} {escape_mdv2(app_cap)} OTP • {escape_mdv2(flag)}\n\n"
        ...
        # ✅ Raw SMS — প্রতিটা line আলাদা করে quote করো
        if raw_sms:
            quoted_lines = "\n".join(
...
async def get_xmint_console_logs(force=False):
    ...
    try:
        res = await xmint_session.get(f"{XMINT_BASE_URL}/mdashboard/console/info")
        if res is None or res.status_code != 200:
            logging.warning(f"X.Mint Console: HTTP {res.status_code if res else 'None'}")
            return _xmint_console_cache["logs"]
        try:
            data = res.json()
        except Exception:
...
async def api_get_number_dual(range_val, app_name="FACEBOOK"):
    ...
    if s1_task.done():
        return s1_task.result()
    if s2_task.done():
        return s2_task.result()
    
    return {"error": "Both panels failed"}
...
def app_select_inline():
    ...
    for app in apps:
        ...
        buttons.append([
            InlineKeyboardButton(f"⟦ {display} • ➊ ⟧", callback_data=f"app_s1_{app}"),
            InlineKeyboardButton(f"⟦ {display} • ➋ ⟧", callback_data=f"app_s2_{app}")
        ])
...
async def safe_edit(query, text, **kwargs):
    ...
    try:
        ...
        # Silently ignore already-edited messages
...
async def auto_otp_multi(message, numbers, user_id, range_val, bot=None):
    ...
    async def _run():
        ...
        # Loading loop — প্রতি 5 সেকেন্ডে random text update
        start_time = asyncio.get_event_loop().time()
        while not stop_event.is_set():
            elapsed = asyncio.get_event_loop().time() - start_time
            # 60 সেকেন্ড timeout — silent
            if elapsed >= 60:
                stop_event.set()
                break
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("app_s1_"):
        ...
        # ✅ আগের message edit করো — নতুন message না
        chat_id = query.message.chat.id
        try:
            await context.bot.edit_message_text(
                chat_id=chat_id,
                message_id=user_msg.get(chat_id, query.message.message_id),
                text="⏳ Getting Number..."
            )
        except Exception:
            loading_msg = await query.message.reply_text("⏳ Getting Number...")
        ...
        if data_r.get("meta", {}).get("code") == 200:
            ...
            if number_session:
                if panel == "S1":
                    ...
                    await xmint_pool.return_number_session(number_session)
            await safe_edit(query, "❌ Number পাওয়া যায়নি!", reply_markup=main_keyboard(user_id))
...
async def post_init(application):
    try:
        ...
        logging.info("✅ S1 pool (50/50) initialized")
    except Exception as e:
        logging.error(f"⚠️ S1 pool init error: {e}")
    
    try:
        # ✅ S2 background এ initialize হবে — startup block করবে না
