    ...
    while not stop_event.is_set():
        ...
        try:
            ...
            if panel == "S1":
                ...
                # status success এ না পেলে সব status এ check করো
                if not nums:
                    data2 = await api_get_info(search=clean_num, status="")
                    if data2.get("meta", {}).get("code") == 200:
                        nums = data2["data"].get("numbers") or []
            else:
                data = await api_get_info_s2(search=clean_num, status="success")
                if data.get("meta", {}).get("code") == 200:
                    nums = data["data"].get("numbers") or []
                if not nums:
                    data2 = await api_get_info_s2(search=clean_num, status="")
                    if data2.get("meta", {}).get("code") == 200:
                        nums = data2["data"].get("numbers") or []
