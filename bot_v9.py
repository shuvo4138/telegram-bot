...
async def api_get_number_s2(range_val, app_name="FACEBOOK"):
    """X.Mint থেকে number নাও - Pool system সহ"""
    ...
    try:
        ...
        if not token:
            logging.error("❌ X.Mint: No session available")
            return {"error": "No X.Mint session available"}
        
        logging.info(f"✅ X.Mint: Got session, requesting number...")
        ...
        result = res.json()
        logging.info(f"✅ X.Mint number response: {res.status_code}, code: {result.get('meta', {}).get('code')}")
        return result
    except Exception as e:
...
async def api_get_info_s2(search="", status=""):
    """X.Mint থেকে OTP info নাও - Pool system সহ"""
...
async def post_init(application):
    ...
    logging.info("✅ Dual Panel Session pools (50/50 each) initialized!")
