...
def _build_otp_post_header(number, app, country, flag, panel):
    ...
    🇬🇳 GN FB 2246***445 [A1]
    — flag + ISO(2) + SVC_SHORT + prefix***last3 [panel]
    """
    clean_num = str(number).replace("+", "").strip()
    # country ISO 2-letter
    country_code = extract_country_code_from_number(clean_num)
    iso = country_code[:2].upper() if country_code and country_code != "Unknown" else country[:2].upper()
    # service short
    app_up = app.upper()
    svc = "FB" if "FACEBOOK" in app_up else ("IG" if "INSTAGRAM" in app_up else ("WA" if "WHATSAPP" in app_up else app_up[:2]))
    # masked number: prefix(first 4) + *** + last 3
    if len(clean_num) > 7:
...
async def job_a1_range_post(context):
    ...
    try:
        ...
        for r in ranges:
            ...
            # ── Header: e.g. 🇹🇿 Tanzania FB 2559743XXX ──
            header_line = f"{range_flag} {country_name} {svc_short} {display_range}"
            text = escape_mdv2(header_line)
    ...
    try:
        ...
        for panel_label, logs in [("S1", s1_logs), ("S2", s2_logs)]:
            ...
            for log in logs:
                ...
                raw_sms_line = f"\n📨 `{escape_mdv2(raw_sms[:120])}`" if raw_sms else ""
                text = (
