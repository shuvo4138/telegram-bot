...
def a2_extract_country_code(rng):
    ...
    for length in [3, 2, 1]:
        ...
        if c in COUNTRY_NAMES_CODE:
            return c
    return clean[:3] if len(clean) >= 3 else clean[:2]
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            if data.get("meta", {}).get("code") == 200:
                ...
                logger.info(f"A2 liveaccess parsed ranges (all services): {len(ranges_out)}")
...
async def a2_get_number(rid):
    ...
    # API accepts range with or without XXX suffix — send as-is (uppercase)
    clean_rid = str(rid).upper().strip()
    try:
        async with httpx.AsyncClient(timeout=20) as client:
            res = await client.post(
...
async def do_get_number_a2(message, user_id, bot=None):
    ...
    rid_clean = range_val.upper().strip()  # Keep XXX — API expects e.g. "22465XXX"
    ...
    for attempt in range(2):
        ...
        if result:
            ...
            # Try all known key names — log whichever key returns the number
            full_num = (
                result.get("no_plus_number") or
                result.get("national_number") or
                result.get("full_number") or
                result.get("phone") or
            ...
            full_num = str(full_num).replace("+", "").strip()
...
async def job_a2_range_post(context):
    ...
    try:
        ...
        for r in ranges:
            ...
            clean_base = rng.replace("XXX", "").replace("X", "").strip()
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "select_panel_A2_fb":
        ...
        logger.info(f"A2 panel select: {len(ranges)} ranges fetched for user {user_id}")
        if not ranges:
            await safe_edit(query,
                "❌ A2_fb তে এখন কোনো active range নেই।\n\nকিছুক্ষণ পর try করুন।",
                reply_markup=InlineKeyboardMarkup([[
                    InlineKeyboardButton("◀️ Back", callback_data="back_app", api_kwargs={"style": "primary"})
                ]])
            )
        ...
        logger.info(f"A2 country_map keys: {list(country_map.keys())}")
        logger.info(f"A2 ranges sample: {[r.get('range') for r in ranges[:3]]}")
        if not country_map:
            await safe_edit(query,
                "❌ A2_fb তে এখন কোনো active range নেই।\n\nকিছুক্ষণ পর try করুন।",
                reply_markup=InlineKeyboardMarkup([[
    ...
    if data.startswith("a2_country:"):
        ...
        for r in ranges:
            ...
            if cname != cname_sel:
                continue
            flag = COUNTRY_FLAGS_CODE.get(code, COUNTRY_FLAGS_CODE.get(code[:2], "🌍"))
            display = rng.upper()  # Already has XXX
            buttons.append([InlineKeyboardButton(
                f"{flag} {display}",
