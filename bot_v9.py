    ...
    try:
        ...
        if res.status_code == 403:
            ...
            if new_session.get("token") and _retry < 2:
                logging.info("🔄 S2: নতুন session দিয়ে retry করা হচ্ছে")
                await asyncio.sleep(1)
        ...
        if result.get("meta", {}).get("code") == 200:
            ...
            if any(k in msg for k in ["block", "rate", "limit", "many", "temporary"]):
                ...
                if _retry < 3:
                    wait_time = 10 * (2 ** _retry)
                    logging.info(f"⏳ S2: {wait_time}s wait করে retry করা হচ্ছে")
                    await asyncio.sleep(wait_time)
                    return await api_get_number_s2(range_val, app_name, _retry=_retry + 1)
                return result, None
...
async def api_get_number(range_val, app_name="FACEBOOK", _retry=0):
    ...
    try:
        ...
        if res.status_code == 403:
            ...
            if new_session.get("token") and _retry < 2:
                logging.info("🔄 S1: নতুন session দিয়ে retry করা হচ্ছে")
                await asyncio.sleep(1)
        ...
        if any(k in msg for k in ["block", "rate", "limit", "many", "temporary"]):
            ...
            if _retry < 3:
                wait_time = 10 * (2 ** _retry)
                logging.info(f"⏳ S1: {wait_time}s wait করে retry করা হচ্ছে")
                await asyncio.sleep(wait_time)
                return await api_get_number(range_val, app_name, _retry=_retry + 1)
            return data, None
        return data, session
...
async def db_load_all_users(bot):
    loaded = 0
    try:
        logging.info("📥 DB Channel থেকে user data load হচ্ছে...")
        ...
        for msg_id in range(max(1, marker_id - 500), marker_id):
            try:
                ...
                continue
        logging.info(f"✅ DB Load complete — {loaded} users loaded")
    except Exception as e:
...
async def app_select_inline_dynamic():
    """Dynamic app buttons — Facebook fixed + S1/S2 থেকে max 5টা random"""
...
def server_select_inline(app_name):
    # Random order — S1/S2 randomly swap হবে
    s1_btn = InlineKeyboardButton("☎️ Facebook S1", callback_data=f"app_s1_{app_name}")
    s2_btn = InlineKeyboardButton("☎️ Facebook S2", callback_data=f"app_s2_{app_name}")
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # প্রথমে Reply Keyboard পাঠাও (Start, Bulk Number, CS Range বাটন দেখাবে)
