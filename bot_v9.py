...
# =============================================
#         X.MINT SESSION POOL (S2 - Auto Login)
# =============================================
...
class XMintSessionPool:
    ...
    async def initialize(self):
        async with self.lock:
            ...
            tasks = [self._login_once() for _ in range(50)]  # 50 sessions for X.Mint
            results = await asyncio.gather(*tasks, return_exceptions=True)
...
async def api_get_number_s2(range_val, app_name="FACEBOOK"):
    """X.Mint থেকে number নাও - Direct call"""
    ...
    if not clean_range:
        logging.error("❌ X.Mint: Invalid range")
    ...
    try:
        logging.info(f"🔵 X.Mint: Logging in...")
        # X.Mint দিয়ে login করি
        async with httpx.AsyncClient(timeout=15) as client:
        ...
        logging.info(f"🔵 X.Mint Login response code: {login_data.get('meta', {}).get('code')}")
        
        if login_data.get("meta", {}).get("code") != 200:
            logging.error(f"❌ X.Mint login failed: {login_data}")
            return {"error": "X.Mint login failed"}
        
        token = login_data["data"].get("token")
        if not token:
            logging.error("❌ X.Mint: No token received")
            return {"error": "No token from X.Mint"}
        
        logging.info(f"✅ X.Mint: Got token, requesting number...")
        
        # Token দিয়ে number নাও
        headers = {
        ...
        result = res.json()
        logging.info(f"✅ X.Mint number response code: {res.status_code}, data: {result}")
        return result
    except Exception as e:
...
async def api_get_info_s2(search="", status=""):
    """X.Mint থেকে OTP info নাও - Direct call"""
    try:
        # X.Mint দিয়ে login করি
        async with httpx.AsyncClient(timeout=15) as client:
            login_res = await client.post(
...
async def send_otp_to_channel(bot, number, otp, app, country, flag, raw_sms=""):
    ...
    """
    try:
        # ⏳ চ্যানেলে OTP পাঠানোর আগে delay (5 সেকেন্ড)
...
async def auto_otp_multi(message, numbers, user_id, range_val, bot=None):
    ...
    if found_otp:
        ...
        found_raw = result_holder.get("raw", "")  # raw SMS নিচ্ছি
        ...
        # OTP Channel এ forward করো - raw SMS সহ
        if bot:
...
async def post_init(application):
    ...
    logging.info("✅ StexSMS Session pool initialized! X.Mint uses direct login.")
