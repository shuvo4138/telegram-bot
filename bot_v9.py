...
# =============================================
#         DATABASE SETUP (Supabase REST API)
# =============================================
...
async def db_init_tables():
    """Supabase REST API — tables already আছে, কিছু করতে হবে না"""
    logging.warning("✅ Supabase REST API ready")

async def db_save_user_async(user_id, data: dict):
    """User data Supabase এ save করো (upsert)"""
    try:
        payload = {
            "user_id": user_id,
...
async def db_mark_posted(unique_id: str):
    ...
    try:
        ...
        logging.error(f"DB mark_posted error: {e}")

async def job_midnight_reset(context):
    """প্রতিদিন রাত 12টায় posted_sms Supabase থেকে delete + memory reset"""
    global _posted_sms_ids, _otp_counter, _otp_counter_date
    try:
        ...
        _otp_counter = {}
        _otp_counter_date = ""
        logging.warning("✅ Midnight reset: posted_sms cleared + OTP counter reset")
    except Exception as e:
        logging.error(f"Midnight reset error: {e}")
...
# Fix #1 — Edit mode (no duplicate post)
LAST_POST_MESSAGE_ID = None
LAST_POST_TEXT = ""

async def send_or_edit_post(bot, text, keyboard):
    global LAST_POST_MESSAGE_ID, LAST_POST_TEXT
    try:
        if LAST_POST_MESSAGE_ID:
            if text != LAST_POST_TEXT:
                try:
                    ...
                    pass  # edit fail হলে নতুন send করবো
        ...
        logging.error(f"send_or_edit_post error: {e}")
    ...
    try:
        ...
        try:
            s1_logs, s2_logs = await asyncio.wait_for(
                asyncio.gather(
                    get_console_logs(force=True),
                    get_xmint_console_logs(force=True),
                    return_exceptions=True
                ),
                timeout=45
            )
            if isinstance(s1_logs, Exception):
                logging.warning(f"S1 logs error: {s1_logs}")
                s1_logs = []
            if isinstance(s2_logs, Exception):
                logging.warning(f"S2 logs error: {s2_logs}")
                s2_logs = []
        except asyncio.TimeoutError:
            logging.error("⚠️ Both S1+S2 logs timeout (45s)")
            return
        ...
        for panel_label, logs in [("S1", s1_logs), ("S2", s2_logs)]:
            for log in logs:
                ...
                # BD time — প্রতি 2 মিনিটে নতুন unique_id (memory only, no Supabase)
                from datetime import timezone, timedelta as _td
                now_bd = datetime.now(timezone(_td(hours=6)))

                range_val = log.get("range", "").strip()
                slot = now_bd.strftime('%Y-%m-%d %H:') + str(now_bd.minute // 2 * 2).zfill(2)
                unique_id = f"range_{range_val}_{slot}"

                if unique_id in _posted_sms_ids:
                    continue

                _posted_sms_ids.add(unique_id)  # শুধু memory তে রাখো
        ...
        if messages_to_send:
            for msg_text, msg_keyboard in messages_to_send:
                ...
                await asyncio.sleep(2)  # Flood avoid
...
def init_user(user_id):
    ...
    if is_new:
        asyncio.create_task(db_save_user_async(user_id, d))
    ...
    while not stop_event.is_set():
        ...
        try:
            ...
            if panel == "S1":
                ...
                # Session না থাকলে pool থেকে নতুন নাও
                if not saved_session or not saved_session.get("token"):
                    saved_session = await session_pool.get_otp_session()
                    if not saved_session or not saved_session.get("token"):
                        await asyncio.sleep(5)
                        continue
                    user_data[user_id]["number_session"] = saved_session
                data = await api_get_info(search=clean_num, status="success", saved_session=saved_session)
                if data.get("meta", {}).get("code") == 200:
                    nums = data["data"].get("numbers") or []
            else:
                # S2 — pool থেকে session নাও, না থাকলে নতুন login
                s2_session = user_data[user_id].get("number_session")
                if not s2_session or not s2_session.get("token"):
                    s2_session = await xmint_pool.get_otp_session()
                    if not s2_session or not s2_session.get("token"):
                        s2_session = await xmint_pool._login_once()
                    if not s2_session or not s2_session.get("token"):
                        await asyncio.sleep(5)
                        continue
                    user_data[user_id]["number_session"] = s2_session
...
async def post_init(application):
    ...
    # ✅ Supabase থেকে সব user load করো
    try:
        async with httpx.AsyncClient(timeout=15) as client:
            res = await client.get(
                f"{SUPABASE_URL}/rest/v1/users",
        ...
        if isinstance(rows, list):
            ...
            logging.warning(f"✅ Supabase: {len(rows)} users loaded")
    except Exception as e:
        logging.error(f"DB load users error: {e}")

    # ✅ posted_sms load করো (duplicate avoid)
    try:
        async with httpx.AsyncClient(timeout=15) as client:
        ...
        if isinstance(rows2, list):
            ...
            logging.warning(f"✅ Supabase: {len(rows2)} posted_sms loaded")
    except Exception as e:
        logging.error(f"DB load posted_sms error: {e}")
    try:
...
if __name__ == "__main__":
    ...
    # ✅ প্রতিদিন রাত 12টায় (BD time) posted_sms reset
    from datetime import timezone, timedelta
    import datetime as _dt
    bd_now = _dt.datetime.now(timezone(timedelta(hours=6)))
    midnight_bd = bd_now.replace(hour=0, minute=0, second=0, microsecond=0) + timedelta(days=1)
