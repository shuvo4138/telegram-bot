...
async def get_live_traffic_data():
    """
    🚦 Live Traffic — S1 + S2 combined
    Returns: {app_name: {total: N, countries: {country: count}}}
    """
...
def build_live_traffic_message(traffic_data):
    """🚦 Live Traffic message build করো"""
    if not traffic_data:
        return "🚦 Live Traffic\n\nNo data available."

    total_all = sum(v["total"] for v in traffic_data.values())
    now = datetime.now().strftime("%I:%M %p")

    lines = ["🚦 *Live Traffic*\n"]
    lines.append(f"📊 Total: {total_all} OTP\n")
    lines.append("━━━━━━━━━━━━━━━━━━━━━━━━\n")
    ...
    for app, data in traffic_data.items():
        ...
        lines.append(f"{emoji} *{app.capitalize()}* — {total} OTP ({pct:.1f}%)\n")
        lines.append(f"`{bar}`\n")
        ...
        for country, count in sorted(countries.items(), key=lambda x: x[1], reverse=True)[:5]:
            ...
            country_parts.append(f"{flag} {count} OTP")
        if country_parts:
            lines.append(f"  {' · '.join(country_parts)}\n")
        lines.append("\n")
...
def build_range_dashboard(app_name, ranges_by_country):
    """Channel এ post করার জন্য range dashboard build করো"""
    if not ranges_by_country:
        return None

    now = datetime.now()
    current_time = now.strftime("%I:%M %p")
    next_time = (now + timedelta(minutes=20)).strftime("%I:%M %p")

    total_ranges = sum(len(v) for v in ranges_by_country.items())
    # Recalculate properly
    total_ranges = sum(len(v) for v in ranges_by_country.values())
    total_countries = len([k for k in ranges_by_country if k not in ("OTHER REGIONS",)])

    app_emoji = APP_EMOJIS.get(app_name.upper(), "📱")
    app_title = f"{app_emoji} {app_name.upper()} RANGE DASHBOARD"
    title_pad = max(0, (44 - len(app_title)) // 2)

    lines = []
    lines.append(f"╔══════════════════════════════════════════╗")
    lines.append(f"║{' ' * title_pad}{app_title}{' ' * title_pad}║")
    lines.append(f"╚══════════════════════════════════════════╝\n")
    lines.append(f"📊 TOTAL COUNTRIES: {total_countries}       📡 TOTAL RANGES: {total_ranges}")
    lines.append(f"⏱ UPDATE INTERVAL: 20 MIN\n")
    lines.append(f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n")

    # Known countries first, OTHER REGIONS last
    ordered = {}
    for k, v in ranges_by_country.items():
        if k not in ("OTHER REGIONS", "PostPaid"):
            ordered[k] = v
    if "PostPaid" in ranges_by_country:
        ordered["PostPaid"] = ranges_by_country["PostPaid"]
    if "OTHER REGIONS" in ranges_by_country:
        ordered["OTHER REGIONS"] = ranges_by_country["OTHER REGIONS"]

    for country, ranges in ordered.items():
        flag = get_flag(country)
        code = get_country_code(country)
        code_display = f"#{code}" if code else ""
        count = len(ranges)
        lines.append(f"🌍 {flag} {country.upper()} {code_display}  [{count}]")
        lines.append(f"──────────────────────────────")
        # Ranges in code format — touch করে copy করা যাবে
        # 4 টা করে একলাইনে
...
# Apps যেগুলোর range post হবে
RANGE_POST_APPS = ["FACEBOOK", "INSTAGRAM", "WHATSAPP", "TELEGRAM"]

async def job_post_live_ranges(context):
    """প্রতি 20 মিনিটে channel এ live ranges post করো — S1+S2 combined"""
    try:
        bot = context.bot
        s1_logs = await get_console_logs(force=True)
        s2_logs = await get_xmint_console_logs(force=True)

        # কোন apps available আছে তা বের করো
        available_apps = set()
        for log in s1_logs + s2_logs:
            app = log.get("app_name", "").replace("*", "").strip().upper()
            if app:
                available_apps.add(app)

        for app_name in RANGE_POST_APPS:
            if app_name not in available_apps:
                continue
            try:
                ranges_by_country = await get_all_ranges_by_country_combined(app_name)
                if not ranges_by_country:
                    continue
                msg = build_range_dashboard(app_name, ranges_by_country)
                if not msg:
                    continue
                ...
                logging.error(f"Range post error [{app_name}]: {e}")

    except Exception as e:
        logging.error(f"job_post_live_ranges error: {e}")
...
if __name__ == "__main__":
    ...
    # ✅ প্রতি 20 মিনিটে channel এ live ranges post করো
    app.job_queue.run_repeating(job_post_live_ranges, interval=1200, first=30)
