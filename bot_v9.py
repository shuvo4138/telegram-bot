...
SESSION_POOL_SIZE = 300  # ⬆️ 100 থেকে 300 - ৩ গুণ বেশি
NUMBER_GET_SLOTS = 150  # ⬆️ 50 থেকে 150
OTP_CHECK_SLOTS = 150   # ⬆️ 50 থেকে 150
...
class SessionPool:
    ...
    async def get_number_session(self):
        try:
            session = await asyncio.wait_for(self.number_sessions.get(), timeout=60)
    ...
    async def get_otp_session(self):
        try:
            session = await asyncio.wait_for(self.otp_sessions.get(), timeout=60)
            if time.time() - session.get("time", 0) > 1500:
                session = await self._login_once()
                if not session.get("token"):
                    session = self.all_sessions[0] if self.all_sessions else {}
...
class XMintSessionPool:
    ...
    async def initialize(self):
        async with self.lock:
            ...
            tasks = [self._login_once() for _ in range(300)]  # ⬆️ 100 থেকে 300
            results = await asyncio.gather(*tasks, return_exceptions=True)
            ...
            for r in results:
                if isinstance(r, dict) and r.get("token"):
                    self.all_sessions.append(r)
                    if number_count < 150:  # ⬆️ 50 থেকে 150
                        await self.number_sessions.put(r)
                        number_count += 1
                    elif otp_count < 150:  # ⬆️ 50 থেকে 150
                        await self.otp_sessions.put(r)
                        otp_count += 1
...
async def api_get_number_s2(range_val, app_name="FACEBOOK"):
    ...
    try:
        ...
        if any(k in msg for k in ["block", "rate", "limit", "many", "temporary"]):
            logging.warning(f"X.Mint Rate limited: {msg}")
...
async def send_otp_to_channel(bot, number, otp, app, country, flag, raw_sms=""):
    try:
        ...
        msg = (
            f"{escape_mdv2(app_cap)} OTP {escape_mdv2(country_code)} {escape_mdv2(flag)}\n\n"
            f"🪙 Number{escape_mdv2(flag)} : {escape_mdv2(hidden_num)}\n"
            f"🔐 Code : `{otp}`\n"
        )
...
async def get_xmint_console_logs(force=False):
    ...
    try:
        ...
        headers = {
            ...
            'Content-Type': "application/json",
            'mauthtoken': token,
            'Cookie': f"mautToken={token}; session_token={session.get('session', '')}"
        }
        async with httpx.AsyncClient(timeout=15) as client:
            res = await client.get(
...
def after_number_inline(number, range_val):
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("🔄 Same Range", callback_data=f"same_{range_val}"),
         InlineKeyboardButton("🔀 Change Range", callback_data="change_range")],
    ])
...
# =============================================
#         ERROR-SAFE EDIT MESSAGE HELPER
# =============================================

async def safe_edit_message(query, text, reply_markup=None):
    """edit_message_text কে safe করা - already edited message error handle করে"""
    try:
        await safe_edit_message(query, text, reply_markup=reply_markup)
    except Exception as e:
        # Message already edited বা delete হয়েছে - ignore করো
        if "message is not modified" in str(e).lower() or "message to edit not found" in str(e).lower():
            logging.debug(f"Message already edited: {e}")
        else:
...
async def auto_otp_multi(message, numbers, user_id, range_val, bot=None):
    ...
    try:
        await asyncio.wait_for(otp_found_event.wait(), timeout=600)
    except asyncio.TimeoutError:
        pass
...
async def do_get_number(message, user_id, count=1, user_name="User", bot=None):
    ...
    if count == 1:
        # ⚡ Dual-panel concurrent fetch for speed
        if panel in ["S1", "S2"]:
            # User specific panel select করেছে
            if panel == "S1":
                data = await api_get_number(range_val, app)
            else:
                data = await api_get_number_s2(range_val, app)
        else:
            # Fallback: দুটো panel একসাথে চালাও
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("app_s1_"):
        ...
        await safe_edit_message(query, f"⏳ {app_name} লোড হচ্ছে...")
        ...
        user_data[user_id]["carrier"] = None
        user_data[user_id]["range"] = None
        await safe_edit_message(query, "⏳ Range লোড হচ্ছে...")
        logs = await get_xmint_console_logs() if panel == "S2" else await get_console_logs()
        seen = set()
        ranges = []
        for log in logs:
        ...
        panel = user_data[user_id].get("panel", "S1")
        user_data[user_id]["country"] = None
        await safe_edit_message(query, "⏳ Loading...")
        ...
        if data_r.get("meta", {}).get("code") == 200:
            ...
            await safe_edit_message(query, "❌ Number পাওয়া যায়নি!", reply_markup=main_keyboard(user_id))
...
async def post_init(application):
    ...
    logging.info("✅ StexSMS Pool (50/50) + X.Mint Pool (50/50) initialized!")
