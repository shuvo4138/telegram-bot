...
class S2SessionPool:
    ...
    async def _login_once(self):
        try:
            ...
            logger.info(f"S2 Login status: {res.status_code}")
            if res.status_code == 403:
                logger.warning(f"S2 Login 403 Forbidden")
                return {}
            if res.status_code != 200:
                logger.warning(f"S2 Login failed: HTTP {res.status_code} | {res.text[:200]}")
                return {}
            try:
                data = res.json()
            except Exception:
                logger.warning(f"S2 Login JSON parse error: {res.text[:200]}")
                return {}
            logger.info(f"S2 Login response meta: {data.get('meta')}")
            if data.get("meta", {}).get("code") == 200:
                token = data["data"].get("token")
...
async def send_otp_to_channel(bot, number, otp, app, country, flag, raw_sms="", panel="S1"):
    try:
        ...
        if _otp_ch_link and len(_otp_ch_link) > 10:
            kb_row.append(InlineKeyboardButton("📢 Main Channel", url=_otp_ch_link, style="primary"))
        if BOT_USERNAME:
...
def admin_keyboard_s1s2():
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("📦 Bulk ON", callback_data="admin_bulk_on", style="primary"),
         InlineKeyboardButton("📦 Bulk OFF", callback_data="admin_bulk_off", style="destructive")],
        [InlineKeyboardButton("👥 All Users", callback_data="admin_allusers", style="secondary"),
         InlineKeyboardButton("📊 Stats", callback_data="admin_stats_s12", style="secondary")],
        [InlineKeyboardButton("🔑 API Status", callback_data="admin_apistatus", style="secondary"),
         InlineKeyboardButton("🔄 Refresh Sessions", callback_data="admin_refresh", style="secondary")],
        [InlineKeyboardButton("📢 Broadcast", callback_data="admin_broadcast_s12")],
    ])

def admin_keyboard_s3():
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("📊 Statistics", callback_data="s3admin_stats", style="secondary")],
        [InlineKeyboardButton("➕ Add Numbers", callback_data="s3admin_addnumbers", style="primary")],
        [InlineKeyboardButton("📢 Broadcast", callback_data="s3admin_broadcast")],
        [InlineKeyboardButton("📈 Analytics", callback_data="s3admin_analytics", style="secondary")],
        [InlineKeyboardButton("🗑️ Delete Numbers", callback_data="s3admin_delete", style="destructive")],
        [InlineKeyboardButton("⚙️ Settings", callback_data="s3admin_settings", style="secondary")],
    ])

async def panel_select_inline():
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("📘 Facebook S1", callback_data="select_panel_S1", style="primary")],
        [InlineKeyboardButton("📘 Facebook S2", callback_data="select_panel_S2", style="primary")],
        [InlineKeyboardButton("📘 Facebook S3", callback_data="select_panel_S3", style="primary")],
    ])

def server_select_inline(app_name):
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("📘 Facebook S1", callback_data=f"app_s1_{app_name}", style="primary")],
        [InlineKeyboardButton("📘 Facebook S2", callback_data=f"app_s2_{app_name}", style="primary")],
        [InlineKeyboardButton("📘 Facebook S3", callback_data=f"app_s3_{app_name}", style="primary")],
    ])
...
def country_select_inline(countries, app_name):
    ...
    if row:
        buttons.append(row)
    buttons.append([InlineKeyboardButton("◀️ Back", callback_data=f"back_app", style="destructive")])
...
def range_select_inline(ranges, app_name, country):
    ...
    for r in ranges[:15]:
        ...
        if not rv.upper().endswith('X'):
            ...
            rv_display = rv
        buttons.append([InlineKeyboardButton(f"📡 {rv_display}", callback_data=f"range_{rv}", style="primary")])
    buttons.append([InlineKeyboardButton("◀️ Back", callback_data=f"back_country_{app_name}", style="destructive")])
    return InlineKeyboardMarkup(buttons)

def after_number_inline_s1s2(number, range_val):
    buttons = [
        [InlineKeyboardButton("🔄 New Number", callback_data=f"new_number_{range_val}", style="primary")],
        [InlineKeyboardButton("🌍 Change Region", callback_data="back_app", style="destructive")],
    ]
...
def after_number_inline_s3(pool_key):
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("🔄 Change Numbers", callback_data=f"s3change:{pool_key}", style="primary")],
        [InlineKeyboardButton("👁 View OTP", url="https://t.me/+SWraCXOQrWM4Mzg9")],
        [InlineKeyboardButton("🌍 Change Country", callback_data="s3changecountry", style="destructive")],
    ])
    ...
    async with _s3_poll_lock:
        try:
            ...
            if _s3_ch_link and len(_s3_ch_link) > 10:
                _s3_kb_buttons.append(InlineKeyboardButton("📢 Main Channel", url=_s3_ch_link, style="primary"))
            if BOT_USERNAME:
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_new and JOIN_CHANNEL_USERNAME:
        ...
        if not ch1_joined or not ch2_joined:
            keyboard_buttons = [
                [InlineKeyboardButton("🔗 Main Channel", url=JOIN_CHANNEL_LINK, style="primary")],
                [InlineKeyboardButton("🔗 OTP Channel", url=OTP_CHANNEL_JOIN_LINK, style="primary")],
                [InlineKeyboardButton("🔗 Backup Channel", url=BACKUP_CHANNEL_LINK, style="primary")],
                [InlineKeyboardButton("✅ Verify", callback_data="verify_join", style="primary")],
            ]
    ...
    if text == "🛟 Support Admin":
        await update.message.reply_text(
            ...
            reply_markup=InlineKeyboardMarkup([[
                InlineKeyboardButton("🛟 Support Admin", url=SUPPORT_ADMIN_LINK, style="primary")
            ]])
        )
        return
...
async def handle_txt_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        parse_mode="Markdown",
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("📘 Facebook", callback_data="pool_service_fb", style="primary")],
            [InlineKeyboardButton("📸 Instagram", callback_data="pool_service_ig", style="primary")],
            [InlineKeyboardButton("📘📸 Facebook + Instagram (Both)", callback_data="pool_service_both", style="primary")],
        ])
    )
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("app_s3_"):
        ...
        buttons.append([InlineKeyboardButton("◀️ Back", callback_data="back_app", style="destructive")])
    ...
    if data.startswith("s3deletepool:"):
        ...
        await safe_edit(query,
            ...
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("✅ হ্যাঁ Delete করো", callback_data=f"s3confirmdeletepool:{pool_key}", style="destructive")],
                [InlineKeyboardButton("❌ Cancel", callback_data="s3admin_delete", style="secondary")]
            ])
        )
        return
