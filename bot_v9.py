...
def parse_pool_key(pool_key):
    ...
    code = parts[0]
    service = parts[-1].lower() if len(parts) >= 2 else "fb"
    return code, service
...
def _s2_load_token_cache():
    try:
        if os.path.exists(S2_TOKEN_CACHE_FILE):
            with open(S2_TOKEN_CACHE_FILE, "r") as f:
                data = json.load(f)
            # valid if saved < 20 min ago
            if time.time() - data.get("saved_at", 0) < 1200:
                return data.get("token"), data.get("session", "")
    except Exception:
        pass
    return None, None

def _s2_save_token_cache(token, session):
    try:
        with open(S2_TOKEN_CACHE_FILE, "w") as f:
            json.dump({"token": token, "session": session, "saved_at": time.time()}, f)
    except Exception:
        pass
...
class S2SessionPool:
    ...
    • Failed session disable + auto-recovery
    • Flood detection log
    • Smart round-robin rotation
    """
    S2_NUM_TARGET  = 40   # number sessions
    S2_OTP_TARGET  = 10   # otp sessions
    S2_TOTAL       = S2_NUM_TARGET + S2_OTP_TARGET
    TOKEN_TTL      = 1200  # seconds (20 min)
    COOLDOWN       = 0.4   # seconds before session reuse (0.3-0.5 range, applied on get)
    MAX_CONCURRENT = 15    # semaphore limit (increased concurrency)

    def __init__(self):
        self.number_sessions  = asyncio.Queue()
        self.otp_sessions     = asyncio.Queue()
        self.all_sessions     = []
        self.disabled         = {}   # token -> disabled_until timestamp
        self._session_locks   = {}   # token -> asyncio.Lock
        self._session_cool    = {}   # token -> last_used timestamp
        self.initialized      = False
        self.lock             = asyncio.Lock()
    ...
    def _get_session_lock(self, token):
        if token not in self._session_locks:
            self._session_locks[token] = asyncio.Lock()
        return self._session_locks[token]

    def _is_disabled(self, session):
        token = session.get("token", "")
        until = self.disabled.get(token, 0)
        return time.time() < until

    def _disable_session(self, session, seconds=300):
        token = session.get("token", "")
        self.disabled[token] = time.time() + seconds
        logger.warning(f"⛔ S2 session disabled for {seconds}s | token={token[:12]}...")

    def _enable_session(self, session):
        token = session.get("token", "")
        self.disabled.pop(token, None)
        logger.info(f"✅ S2 session re-enabled | token={token[:12]}...")

    async def _apply_cooldown(self, session):
        """Apply reuse cooldown — called when GETTING a session, not returning it."""
        token = session.get("token", "")
        last = self._session_cool.get(token, 0)
            ...
            for i in range(self.S2_TOTAL - (1 if cached_token else 0)):
                ...
                if r.get("token"):
                    ...
                    if number_count < self.S2_NUM_TARGET:
                        ...
                        await self.otp_sessions.put(r)
                        otp_count += 1
                    # save first token to cache
                    if number_count == 1 and not cached_token:
                        _s2_save_token_cache(r["token"], r.get("session", ""))
                else:
                    # Login failed (403/429) — longer pause before next attempt
                    logger.warning(f"  S2 warmup login failed [{i+1}] — pausing 10s before next")
                    await asyncio.sleep(10)
                    continue
                # staggered random delay (3-6s) — prevents 403 login flood
    ...
    async def _login_once(self, _retry=0):
        try:
            ...
            if res.status_code == 429:
                wait = 30 * (2 ** _retry)
                logger.warning(f"⚠️  S2 Login FLOOD detected — backoff {wait}s (retry {_retry})")
                await asyncio.sleep(wait)
                if _retry < 4:
                    return await self._login_once(_retry + 1)
                return {}
            ...
            if res.status_code != 200:
                ...
                if _retry < 2:
                    await asyncio.sleep(5 * (2 ** _retry))
                    return await self._login_once(_retry + 1)
                return {}
            try:
                data = res.json()
    ...
    async def _get_valid_session(self, queue, pool_name):
        ...
        for _ in range(queue.qsize() + 1):
            try:
                ...
                break
            if self._is_disabled(session):
                asyncio.create_task(self._recover_session(session, queue))
                continue
            if time.time() - session.get("time", 0) > self.TOKEN_TTL:
                # Token expired — refresh quietly (not during warmup)
                new_s = await self._login_once()
                if new_s.get("token"):
                    session = new_s
                else:
                    session["time"] = time.time()  # reuse, reset timer
            # Small cooldown before reuse (0.3-0.5s) — prevents burst on same session
            await self._apply_cooldown(session)
            return session

        # Queue empty after warmup — wait for a session to become available
        logger.warning(f"⚠️ S2 {pool_name} queue empty — waiting for available session")
        try:
            session = await asyncio.wait_for(queue.get(), timeout=30)
            return session
        except asyncio.TimeoutError:
            # Last resort: use any non-disabled session from all_sessions
            logger.error(f"❌ S2 {pool_name} wait timeout — using fallback session")
            for s in self.all_sessions:
                if not self._is_disabled(s):
                    return s
            return {}

    async def _recover_session(self, session, queue, delay=300):
        """Auto re-enable disabled session after delay."""
        await asyncio.sleep(delay)
        self._enable_session(session)
        session["time"] = time.time()
        await queue.put(session)
        logger.info("♻️  S2 session recovered and returned to pool")

    async def get_number_session(self):
        return await self._get_valid_session(self.number_sessions, "Number")

    async def get_otp_session(self):
        return await self._get_valid_session(self.otp_sessions, "OTP")

    async def return_number_session(self, session, failed=False):
        if not session or not session.get("token"):
            return
        if failed:
            self._disable_session(session, seconds=300)
            asyncio.create_task(self._recover_session(session, self.number_sessions))
            return
        # Update last-used timestamp but return instantly (no blocking sleep)
        self._session_cool[session.get("token", "")] = time.time()
        await self.number_sessions.put(session)
    ...
    async def return_otp_session(self, session, failed=False):
        ...
        if failed:
            self._disable_session(session, seconds=300)
            asyncio.create_task(self._recover_session(session, self.otp_sessions))
            return
        await self.otp_sessions.put(session)
...
def admin_keyboard_s1s2():
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("👥 All Users", callback_data="admin_allusers", api_kwargs={"style": "primary"}),
...
def admin_keyboard_s3():
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("➕ Add Numbers", callback_data="s3admin_addnumbers", api_kwargs={"style": "primary"})],
...
async def panel_select_inline():
    ...
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton(f"📜 Facebook S3 ({fb_count})", callback_data="select_panel_S3", api_kwargs={"style": "secondary"})],
        [InlineKeyboardButton(f"📸 Instagram S3 ({ig_count})", callback_data="select_panel_S3_IG", api_kwargs={"style": "secondary"})],
    ])
...
def server_select_inline(app_name):
    ...
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("📗 Facebook S2", callback_data=f"app_s2_{app_name}", api_kwargs={"style": "success"})],
        [InlineKeyboardButton(f"📜 Facebook S3 ({fb_count})", callback_data=f"app_s3_{app_name}", api_kwargs={"style": "secondary"})],
        [InlineKeyboardButton(f"📸 Instagram S3 ({ig_count})", callback_data=f"app_s3ig_{app_name}", api_kwargs={"style": "secondary"})],
    ])
...
async def auto_otp_multi(message, numbers, user_id, range_val, bot=None):
    ...
    async def _return_sessions():
        """Instantly return all held sessions back to pool — no leak, no timeout."""
        held = user_data[user_id].get("number_sessions", [])
        for sess in held:
            if sess and sess.get("token"):
                try:
                    ...
                    logger.error(f"Session return error uid={user_id}: {e}")
        user_data[user_id]["number_sessions"] = []
    ...
    async def _run():
        ...
        try:
            while not all_done():
                ...
                if active:
                    await asyncio.gather(*[poll_number(c) for c in active], return_exceptions=True)
            ...
            # Number changed / panel switched / user cancelled — instant session return
            logger.info(f"OTP task cancelled for uid={user_id} — releasing sessions instantly")
        finally:
            # ALWAYS runs — guarantees session recycle even on exception/cancel/timeout
            await _return_sessions()
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    inline_kb = InlineKeyboardMarkup([
        [InlineKeyboardButton("📘 Facebook S1", callback_data="select_panel_S1", api_kwargs={"style": "primary"})],
        [InlineKeyboardButton("📗 Facebook S2", callback_data="select_panel_S2", api_kwargs={"style": "success"})],
        [InlineKeyboardButton(f"📜 Facebook S3 ({fb_count})", callback_data="s3app:fb", api_kwargs={"style": "secondary"})],
        [InlineKeyboardButton(f"📸 Instagram S3 ({ig_count})", callback_data="s3app:ig", api_kwargs={"style": "secondary"})],
    ])
        ...
        if ch1_joined and ch2_joined:
            ...
            new_msg = await context.bot.send_message(
                ...
                reply_markup=InlineKeyboardMarkup([
                    ...
                    [InlineKeyboardButton("📙 Facebook S3", callback_data="select_panel_S3", api_kwargs={"style": "primary"})],
                ])
            )
            user_msg[chat_id] = new_msg.message_id
        else:
