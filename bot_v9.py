...
def safe_channel_button(label="📢 Main Channel"):
    ...
    if link and len(link) > 10:
        return InlineKeyboardButton(label, url=link, api_kwargs={"style": "secondary"})
    fallback_url = OTP_CHANNEL_LINK or CHANNEL2_LINK or ""
    if fallback_url and len(fallback_url) > 10:
        return InlineKeyboardButton(label, url=fallback_url, api_kwargs={"style": "secondary"})
    return InlineKeyboardButton(label, callback_data="noop_channel", api_kwargs={"style": "secondary"})
...
async def send_otp_to_channel(bot, number, otp, app, country, flag, raw_sms="", panel="S1"):
    try:
        ...
        if _otp_ch_link and len(_otp_ch_link) > 10:
            kb_row.append(InlineKeyboardButton("📢 Main Channel", url=_otp_ch_link, api_kwargs={"style": "secondary"}))
        if BOT_USERNAME:
    ...
    async with _s3_poll_lock:
        try:
            ...
            if _s3_ch_link and len(_s3_ch_link) > 10:
                _s3_kb_buttons.append(InlineKeyboardButton("📢 Main Channel", url=_s3_ch_link, api_kwargs={"style": "secondary"}))
            if BOT_USERNAME:
        ...
        for panel_label, logs in [("S1", s1_logs), ("S2", s2_logs)]:
            ...
            for log in logs:
                ...
                if _main_ch_link and len(_main_ch_link) > 10:
                    _kb_buttons.append(InlineKeyboardButton("📢 Main Channel", url=_main_ch_link, api_kwargs={"style": "secondary"}))
                if BOT_USERNAME:
