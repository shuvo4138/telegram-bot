...
class XMintSession:
    def __init__(self):
        self.client = httpx.AsyncClient(
            headers={
                ...
                "mauthtoken": XMINT_HARDCODED_TOKEN,
                "cookie": f"mauthtoken={XMINT_HARDCODED_TOKEN}",
    ...
    async def ensure_logged_in(self):
        pass  # Token hardcoded — no login needed
...
class XMintSessionPool:
    ...
    async def initialize(self):
        async with self.lock:
            if self.initialized:
                return
            logging.info("✅ X.Mint session ready (hardcoded token)")
            self.initialized = True
