...
def supabase_save_user(user_id, data: dict):
...
def supabase_is_posted(unique_id: str) -> bool:
    """Supabase এ posted_sms check করো (duplicate avoid)"""
    try:
        res = supabase.table("posted_sms").select("unique_id").eq("unique_id", unique_id).execute()
        return len(res.data) > 0
    except Exception:
        return False

def supabase_mark_posted(unique_id: str):
    """Supabase এ posted_sms insert করো"""
    try:
        supabase.table("posted_sms").insert({"unique_id": unique_id}).execute()
    except Exception as e:
        logging.error(f"Supabase mark_posted error: {e}")
    ...
    try:
        ...
        for panel_label, logs in [("S1", s1_logs), ("S2", s2_logs)]:
            for log in logs:
                ...
                if supabase_is_posted(unique_id):
                    continue

                supabase_mark_posted(unique_id)
...
def init_user(user_id):
    ...
    # নতুন user হলে Supabase এ save করো
    if is_new:
        supabase_save_user(user_id, d)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_new_user:
        supabase_save_user(user_id, user_data[user_id])
...
async def post_init(application):
    ...
    try:
        ...
        logging.warning(f"✅ Supabase: {len(res.data)} users loaded")
    except Exception as e:
        logging.error(f"Supabase load error: {e}")

    # ✅ Supabase থেকে posted_sms load করো (duplicate avoid)
    try:
        res2 = supabase.table("posted_sms").select("unique_id").execute()
        for row in res2.data:
            _posted_sms_ids.add(row["unique_id"])
        logging.warning(f"✅ Supabase: {len(res2.data)} posted_sms loaded")
    except Exception as e:
        logging.error(f"Supabase posted_sms load error: {e}")
    try:
