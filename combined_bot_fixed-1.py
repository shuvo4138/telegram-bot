...
BOT_USERNAME = os.getenv("BOT_USERNAME", "NumberPanelBot").strip()
...
class S1SessionPool:
    ...
    async def get_number_session(self):
        try:
            ...
            if new_session.get("token"):
                return new_session
            return self.all_sessions[0] if self.all_sessions else {}
    ...
    try:
        ...
        for panel_label, logs in [("S1", s1_logs), ("S2", s2_logs)]:
            for log in logs:
                ...
                raw_sms = (log.get("message") or log.get("sms") or log.get("text") or "").strip()
                ...
                text = (
                    f"{flag} *{escape_mdv2(country)}* — Facebook\n"
                    f"📡 Range: `{escape_mdv2(display_range)}`\n"
                    f"🔑 OTP: `{otp}`\n"
                    f"🖥 Panel: {panel_label}\n"
                    f"🕐 {now_bd.strftime('%H:%M:%S')}"
                )
                keyboard = InlineKeyboardMarkup([[
                    InlineKeyboardButton("🤖 Number Bot", url=f"https://t.me/{BOT_USERNAME}"),
                ]])
...
def main():
    ...
    # S1/S2 Live SMS post — every 60 seconds
