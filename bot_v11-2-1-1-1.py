...
async def do_get_number_a1(message, user_id, bot=None):
    """A1 (ZENEX) — 2টা number নেওয়া + S3 style card + OTP polling."""
    init_user(user_id)
    ...
    card_text = (
        f"✅ *Numbers Assigned\\!*\n\n"
        f"Service: {escape_mdv2(service_name)}\n"
        f"🌍 Country: {flag} {escape_mdv2(country)}\n"
        f"⏳ Reserved: 20 min\n\n"
        f"📩 OTPs forwarded automatically\\.\n\n"
        f"{escape_mdv2(num_lines.strip())}"
    )

    kb_buttons = [
        [InlineKeyboardButton("🔄 Change Numbers", callback_data="a1_change_numbers", api_kwargs={"style": "primary"})],
        [InlineKeyboardButton("🌍 Change Range", callback_data="select_panel_A1_fb", api_kwargs={"style": "primary"})],
    ]
    if OTP_CHANNEL_LINK:
        kb_buttons.append([InlineKeyboardButton("📢 OTP Channel", url=OTP_CHANNEL_LINK, api_kwargs={"style": "success"})])

    try:
        card_msg = await bot.edit_message_text(
            card_text,
            chat_id=chat_id,
    ...
    # OTP polling — 20 min, 5 sec interval
    ...
    async def poll_a1():
        while time.time() < deadline:
            for num_data in numbers:
                ...
                if full in otp_received:
                    continue
                entry = await zenex_poll_otp(full)
                if entry:
                    otp_text = entry.get("otp", "")
                    otp_code = extract_otp(otp_text) or otp_text[:10]
                    otp_received[full] = otp_code
                    # User inbox notification
                    try:
                        inbox_msg = (
                            f"🔔 *OTP এসেছে\\!*\n\n"
                            f"🆕 {escape_mdv2(service_name)} \\[A1\\] 🟢\n\n"
                            f"📱 *Number:* `{escape_mdv2(full)}`\n"
                            f"🔐 *OTP:* `{escape_mdv2(otp_code)}`\n\n"
                            f"📩 {escape_mdv2(otp_text[:200])}"
                        )
                    ...
                    # OTP channel notification
                    try:
                        c_flag = get_flag_by_iso(entry.get("country", country))
                        c_name = entry.get("country", country)
                        ch_msg = (
                            f"🆕 NEW OTP — {escape_mdv2(service_name)} \\[A1\\]\n\n"
                            f"{c_flag} {escape_mdv2(c_name)}\n\n"
                            f"📱 `{escape_mdv2(full)}`\n"
                            f"🔑 `{escape_mdv2(otp_code)}`\n\n"
                            f"━━━━━━━━━━━━━━━\n📩 SMS:\n>{escape_mdv2(otp_text[:300])}"
                        )
                        otp_ch_kb = InlineKeyboardMarkup([[safe_channel_button("📢 Main Channel")]])
                        await bot.send_message(OTP_CHANNEL_ID, ch_msg, parse_mode="MarkdownV2", reply_markup=otp_ch_kb)
                    except Exception as e:
                        logger.error(f"A1 channel send error: {e}")
            if len(otp_received) >= len(numbers):
                break
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("select_panel_"):
        ...
        if panel == "S3":
            ...
            # WhatsApp — S1 এবং S2 উভয় panel থেকে WhatsApp range দেখাও
            app_name = "WHATSAPP"
    ...
    if data == "a1_change_numbers":
        ...
        if not range_val:
            ...
            return
        await safe_edit(query, "⏳ নতুন number নেওয়া হচ্ছে...")
