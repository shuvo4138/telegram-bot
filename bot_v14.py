...
RANGE_POST_APPS_20MIN = ["FACEBOOK"]
RANGE_POST_APPS_1HR = ["TELEGRAM", "WHATSAPP"]

async def _post_ranges_for_apps(bot, app_list, s1_logs, s2_logs, interval_min=20):
    """Helper — given app list এর জন্য range post করো"""
    available_apps = set()
            ...
            for panel_label, logs in [("S1", s1_logs), ("S2", s2_logs)]:
                for log in logs:
                    ...
                    if not country or country.lower() in ["unknown", ""]:
                        ...
                        country = "PostPaid"
                    if country not in result:
                        result[country] = {"S1": [], "S2": []}
                    if range_val not in result[country][panel_label]:
                        result[country][panel_label].append(range_val)

            if not result:
                continue
            msg = build_range_dashboard(app_name, result, interval_min=interval_min)
            if not msg:
                continue
...
async def job_post_live_ranges(context):
    """প্রতি 20 মিনিটে channel এ FACEBOOK range post করো"""
    try:
        bot = context.bot
        s1_logs = await get_console_logs(force=True)
        s2_logs = await get_xmint_console_logs(force=True)
        await _post_ranges_for_apps(bot, RANGE_POST_APPS_20MIN, s1_logs, s2_logs, interval_min=20)
    except Exception as e:
        logging.error(f"job_post_live_ranges error: {e}")

async def job_post_live_ranges_1hr(context):
    """প্রতি 1 ঘন্টায় TELEGRAM + WHATSAPP range post করো"""
    try:
        bot = context.bot
        s1_logs = await get_console_logs(force=True)
        s2_logs = await get_xmint_console_logs(force=True)
        await _post_ranges_for_apps(bot, RANGE_POST_APPS_1HR, s1_logs, s2_logs, interval_min=60)
    except Exception as e:
        logging.error(f"job_post_live_ranges_1hr error: {e}")
...
if __name__ == "__main__":
    ...
    # ✅ প্রতি 20 মিনিটে FACEBOOK range post
    app.job_queue.run_repeating(job_post_live_ranges, interval=1200, first=30)
    # ✅ প্রতি 1 ঘন্টায় TELEGRAM + WHATSAPP range post
    app.job_queue.run_repeating(job_post_live_ranges_1hr, interval=3600, first=60)
