...
async def a2_poll_otp(target_number, seen_otp_ids=None, rid=None):
    """VOLTX OTP poll — GET /success-otp, rid বা number দিয়ে filter।
    seen_otp_ids: set of already-processed otp_id — duplicate skip।
    rid: allocated rid from getnum response — primary filter।
    """
    try:
        params = {}
        if rid:
        ...
        if res.status_code == 200:
            ...
            logger.info(f"A2 success-otp status: {res.status_code} | rid: {rid} | otps: {len(otps)}")
            if data.get("meta", {}).get("code") == 200:
                clean_target = str(target_number).lstrip("+").strip()
                for entry in otps:
                    # number key fallback — VOLTX API key name varies
                    num = str(
                        entry.get("number") or
                        entry.get("no_plus_number") or
                        entry.get("phone") or
                        entry.get("msisdn") or
                        entry.get("national_number") or
                        ""
                    ).lstrip("+").strip()
                    otp_id = entry.get("otp_id", "")
                    # duplicate skip
                    if seen_otp_ids is not None and otp_id in seen_otp_ids:
                        continue
                    # rid match হলে সরাসরি return, নাহলে number match
                    if rid and entry.get("rid") == rid:
                        logger.info(f"A2 OTP matched by rid: {rid} | num: {num}")
                        return entry
                    num_digits = re.sub(r'\D', '', num)
                    target_digits = re.sub(r'\D', '', clean_target)
                    if num_digits and target_digits and (
                        num_digits == target_digits or
                        num_digits.endswith(target_digits[-8:]) or
                        target_digits.endswith(num_digits[-8:])
                    ):
                        logger.info(f"A2 OTP matched by number: {num} → {clean_target}")
                        return entry
        return None
    except Exception as e:
...
async def auto_otp_a2(user_id, numbers, rids, display_range, bot, chat_id):
    ...
    TIMEOUT = 600
    start = time.time()
    received = set()      # number → done
    seen_otp_ids = set()  # otp_id → already processed
    ...
    while True:
        ...
        for idx, number in enumerate(list(numbers)):
            ...
            try:
                ...
                entry = await a2_poll_otp(number, seen_otp_ids=seen_otp_ids, rid=_rid)
                if not entry:
                    continue
