...
async def job_a1_range_post(context):
    ...
    try:
        ...
        for r in ranges:
            ...
            # country flag
            code = display_range[:3] if len(display_range) >= 3 else display_range[:2]
            range_flag = COUNTRY_FLAGS_CODE.get(code, COUNTRY_FLAGS_CODE.get(code[:2], "🌍"))
            country_name = COUNTRY_NAMES_CODE.get(code, COUNTRY_NAMES_CODE.get(code[:2], ""))
            country_line = f"{range_flag} {escape_mdv2(country_name)}\n\n" if country_name else ""
            ...
            # copy box buttons — range + otp
            kb_rows = []
            kb_rows.append([
                InlineKeyboardButton(f"📞 {display_range}", copy_text=CopyTextButton(text=display_range), api_kwargs={"style": "success"}),
            ])
            kb_rows.append([
                InlineKeyboardButton(f"🔐 {otp}", copy_text=CopyTextButton(text=otp), api_kwargs={"style": "success"}),
            ])
            bottom_row = []
            if OTP_CHANNEL_LINK and len(OTP_CHANNEL_LINK) > 10:
                bottom_row.append(InlineKeyboardButton("📢 Main Channel", url=OTP_CHANNEL_LINK, api_kwargs={"style": "primary"}))
            if BOT_USERNAME:
