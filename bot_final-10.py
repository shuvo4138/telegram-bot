    ...
    async def _login_once(self):
        try:
            ...
            if res.status_code == 403:
                logging.error(f"X.Mint: 403 Forbidden - Invalid credentials or blocked account")
                return {}
            ...
            try:
                ...
                logging.error(f"X.Mint: Invalid JSON response - {e}")
                return {}
...
async def send_otp_to_channel(bot, number, otp, app, country, flag, raw_sms=""):
    try:
        ...
        keyboard = InlineKeyboardMarkup([
            [InlineKeyboardButton("📢 Check OTP (Channel)", url=CHANNEL_LINK),
            ...
            for n in nums:
                ...
                if clean_num in api_num or api_num in clean_num:
                    ...
                    if otp and otp not in seen_otps:
                        ...
                        # ✅ PostPaid/Other হলে user এর saved country use করো
...
async def auto_otp_multi(message, numbers, user_id, range_val, bot=None):
    ...
    async def on_otp(otp, n, raw_otp, found_country, found_app):
        ...
        # ✅ User কে same message এ update করো
    ...
    async def _run():
        ...
        inner_tasks = [
            asyncio.create_task(auto_otp_single(num, user_id, stop_event, on_otp))
            for num in numbers
        ]
    ...
    try:
        ...
        if chat_id in user_msg:
            try:
                ...
                sent_message = await message.reply_text("⏳ Getting Number...")
        else:
            sent_message = await message.reply_text("⏳ Getting Number...")
        ...
        await asyncio.sleep(1)
        await sent_message.edit_text("🔍 Checking Panel...")
        await asyncio.sleep(1)
...
async def do_get_number(message, user_id, count=1, user_name="User", bot=None):
    ...
    if count == 1:
        # ✅ আগে loading message পাঠাও — id save করো
        chat_id = message.chat.id
        try:
            if chat_id in user_msg:
                try:
                    loading_msg = await message.get_bot().edit_message_text(
                        chat_id=chat_id,
                        message_id=user_msg[chat_id],
                        text="⏳ Getting Number..."
                    )
        ...
        # ⚡ Always use dual-panel concurrent for maximum speed
        data = await api_get_number_dual(range_val, app)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ✅ Loading animation
    msg = await update.message.reply_text("⏳ Starting Panel...", reply_markup=main_keyboard(user_id))
    await asyncio.sleep(1)
    await msg.edit_text("⚙️ Loading System...")
    await asyncio.sleep(1)
    await msg.edit_text("🔐 Connecting Server...")
    await asyncio.sleep(1)
...
async def post_init(application):
    ...
    try:
        ...
        logging.info("✅ S2 pool (50/50) initialized")
    except Exception as e:
