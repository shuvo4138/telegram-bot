                    ...
                    for item in raw_data:
                        if isinstance(item, dict):
                            ...
                            ranges_out.append({
                                "range": rng_upper,
                                "rid": rng_upper,
                                "service": item.get("service", item.get("sid", "")),
...
async def a2_poll_otp(target_number, seen_otp_ids=None, assign_time_ms=None):
    """VOLTX OTP poll — GET /success-otp, number দিয়ে filter।
    assign_time_ms: number assign হওয়ার unix-ms — এর আগের OTP ignore।
    Doc: time = unix-ms, number = digits only (no plus)
    """
    try:
        async with httpx.AsyncClient(timeout=15) as client:
            res = await client.get(
        ...
        if res.status_code != 200:
            logger.warning(f"A2 success-otp HTTP {res.status_code}")
            return None

        data = res.json()
        if data.get("meta", {}).get("code") != 200:
            logger.warning(f"A2 success-otp meta error: {data.get('meta')}")
            return None

        otps_raw = data.get("data", {})
        otps = otps_raw.get("otps", []) if isinstance(otps_raw, dict) else []
        logger.info(f"A2 success-otp: {len(otps)} otps | target: {target_number}")

        clean_target = re.sub(r'\D', '', str(target_number))

        for entry in otps:
            # ── otp_id: empty string হলে None treat করো — "" set এ থাকলে সব future entry skip হয় ──
            otp_id     = entry.get("otp_id") or None
            entry_time = int(entry.get("time", 0) or 0)

            # ── Timestamp check (doc confirms ms) ──
            if assign_time_ms and entry_time > 0:
                # assign_time_ms থেকে ৩০ সেকেন্ড আগের OTP ignore (넉넉한 clock drift + delivery buffer)
                if entry_time < (assign_time_ms - 30000):
                    logger.info(f"A2 OTP skip (old): otp_time={entry_time} assign_time={assign_time_ms}")
                    continue

            # ── Duplicate skip — otp_id None হলে skip guard বাদ ──
            if seen_otp_ids is not None and otp_id and otp_id in seen_otp_ids:
                continue

            # ── Number match — doc confirms /success-otp এ শুধু "number" field (plain digits) ──
            ...
            if num and clean_target and (
                ...
                logger.info(f"A2 OTP matched! num={num} | otp_id={otp_id} | time={entry_time}")
                return entry
...
async def auto_otp_a2(user_id, numbers, rids, display_range, bot, chat_id):
    ...
    TIMEOUT = 600
    start = time.time()
    # number assign হওয়ার সময়ের আগের OTP ignore — unix-ms
    # ৫ সেকেন্ড আগ থেকে accept করো — delivery delay / clock drift buffer
    assign_time_ms = int((start - 5) * 1000)
    received = set()
    seen_otp_ids = set()
    ...
    while True:
        ...
        for idx, number in enumerate(list(numbers)):
            if number in received:
                continue
            try:
                entry = await a2_poll_otp(number, seen_otp_ids=seen_otp_ids, assign_time_ms=assign_time_ms)
                if not entry:
                    continue
                # doc confirms: /success-otp এ শুধু "message" field
                ...
                received.add(number)
                # otp_id খালি/None হলে number+otp দিয়ে unique key বানাও
                # empty string "" seen_otp_ids এ add করলে সব future entry skip হয় — তাই guard
                _otp_id_raw = entry.get("otp_id") or None
...
async def send_otp_to_channel(bot, number, otp, app, country, flag, raw_sms="", panel="S1"):
    try:
        ...
        panel_label = str(panel).upper() if panel else "S1"
        msg = (
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ── Panel select ──
    if data.startswith("select_panel_") and data not in ("select_panel_A2_fb", "select_panel_S2"):
    ...
    if data.startswith("a2_range:"):
        ...
        user_data[user_id]["panel"] = "A2"
        # Country code + name save করো — range select এর সময়
