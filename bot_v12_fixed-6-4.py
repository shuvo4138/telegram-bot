...
async def a2_poll_otp(target_number, seen_otp_ids=None):
    ...
    try:
        ...
        if res.status_code == 200:
            data = res.json()
            logger.info(f"A2 success-otp status: {res.status_code} | otps: {len(data.get('data',{}).get('otps',[]) if isinstance(data.get('data'),dict) else [])}")
            if data.get("meta", {}).get("code") == 200:
                otps = data.get("data", {}).get("otps", [])
                ...
                for entry in otps:
                    ...
                    if seen_otp_ids is not None and otp_id in seen_otp_ids:
                        continue
                    if num == clean_target or num.endswith(clean_target[-7:]) or clean_target.endswith(num[-7:]):
                        return entry
        return None
    except Exception as e:
    ...
    while True:
        ...
        for number in list(numbers):
            if number in received:
                continue
            try:
                entry = await a2_poll_otp(number, seen_otp_ids=seen_otp_ids)
                if not entry:
                    continue
