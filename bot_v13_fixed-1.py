...
async def a2_poll_otp(target_number, seen_otp_ids=None, start_time_ms=None):
    """VOLTX OTP poll — GET /success-otp, number দিয়ে filter।
    seen_otp_ids : set of already-processed otp_id (number+time) — duplicate skip।
    start_time_ms: bot start হওয়ার unix-ms — এর আগের OTP ignore।
    API doc: number key = plain digits (no plus), otp_id = number+time(ms)
    """
    try:
        async with httpx.AsyncClient(timeout=15) as client:
            res = await client.get(
            ...
            if data.get("meta", {}).get("code") == 200:
                ...
                for entry in otps:
                    ...
                    if seen_otp_ids is not None and otp_id and otp_id in seen_otp_ids:
                        continue
                    # পুরনো OTP skip — bot start এর আগের
                    if start_time_ms and entry.get("time", 0) < start_time_ms:
                        continue
                    # number match — API doc এ key = "number", plain digits
                    ...
                    if num and clean_target and (
                        ...
                        logger.info(f"A2 OTP matched: {num} | otp_id: {otp_id}")
                        return entry
        return None
    except Exception as e:
...
async def auto_otp_a2(user_id, numbers, rids, display_range, bot, chat_id):
    ...
    TIMEOUT = 600
    start = time.time()
    start_time_ms = int(start * 1000)  # unix-ms — এর আগের OTP ignore
    ...
    while True:
        ...
        for idx, number in enumerate(list(numbers)):
            if number in received:
                continue
            try:
                entry = await a2_poll_otp(number, seen_otp_ids=seen_otp_ids, start_time_ms=start_time_ms)
                if not entry:
                    continue
