...
async def job_post_live_sms(context):
    ...
    try:
        bot = context.bot
        # NOTE: S2 panel ইচ্ছাকৃতভাবে ব্যবহার হচ্ছে না, তাই এখানে কিছু করার নেই।
        s2_logs = []
...
def init_user(user_id):
    """user_data[user_id] না থাকলে বানায়, সব key-র default বসায়। আগের ডাটা থাকলে ছোঁবে না (setdefault)।"""
    ...
    d.setdefault("panel", "S2")
    d.setdefault("country", None)
    d.setdefault("country_r", None)
    d.setdefault("range", None)
    d.setdefault("last_number", None)
    d.setdefault("waiting_for", None)
    d.setdefault("otp_active", False)
    d.setdefault("otp_running", False)
    d.setdefault("auto_otp_cancel", False)
    d.setdefault("number_session", None)
    d.setdefault("number_sessions", [])
    d.setdefault("numbers", [])
    d.setdefault("a1_numbers", [])
    d.setdefault("a1_otp_received", {})
    d.setdefault("a1_service", "Facebook New Account")
    d.setdefault("a1_zenex_service", "Facebook")
    d.setdefault("s3_service", "FACEBOOK")
    d.setdefault("tried_ranges", set())
    return d
...
def main():
    ...
    # NOTE: job_whatsapp_range_post ফাংশনটা কোথাও define করা ছিল না — এটাই ক্র্যাশের কারণ।
    # আপাতত disable করে দেওয়া হলো যাতে বট চালু থাকে। ফিচারটা বানাতে চাইলে বলবেন।
